...
- **关联（correlation）**。OpenTelemetry：`src/instrumentation.ts` 作为 `src/index.ts` 的**第一个 import** 初始化（HTTP 埋点须在建服务器前生效）。trace_id/span_id 由 `infrastructure/logger` 的 pino `mixin` 实时从活动 span 注入每行日志（不依赖模块 patch）。Fastify `genReqId` 对齐 trace_id。**BullMQ 跨 Redis** 用 `workers/job-tracing.ts`：`queues.ts` 在每次 `.add` 注入 trace context，worker 必须用 `tracedWorker(...)`，高频 repeatable tick worker（调度器/清扫器）必须用 `tracedTickWorker(...)`（**禁止裸 `new Worker`**，`scripts/guard-trace-propagation.mjs` 强制）以续接 trace；tick worker 空转 tick 零 span（P-TICK 政策，见 job-tracing.ts）。
- **导出全部 env 驱动、零硬编码**（开源仓库要求）。OTel SDK 在两种配置下（有/无 Sentry）都拥有采样、埋点与 OTLP 导出：`OTEL_EXPORTER_OTLP_ENDPOINT`(+`_TRACES_ENDPOINT`)（→ Alloy/Tempo）、`OTEL_SERVICE_NAME`、`OTEL_TRACES_SAMPLER`(`_ARG`)（OTLP 量的唯一旋钮）、`OTEL_SDK_DISABLED`、`OTEL_LOG_LEVEL`、`SYNAPSE_TRACE_FIRST_PARTY_HOSTS`（出口第一方门控）。Sentry（`SENTRY_DSN`/`SENTRY_ENVIRONMENT`）默认**只作错误捕获**，不改变 OTLP 量与埋点集；span 转发是 opt-in：`SENTRY_TRACES_SAMPLE_RATE` 为 Sentry **FORWARD 率**（默认 0 = errors-only；>0 时 event 级按 trace-id 确定性转发进 Sentry Performance，绝不影响 OTLP/Tempo 量）。完整 env 表见 `packages/api/src/instrumentation.ts` 文件头。
