...
function statusNote(
  ...
  handlers: Pick<
    StatusPanelProps,
    "onOpenPairing" | "onOpenServers" | "onStart"
  >
): GuideStep[] {
  const canStart =
    Boolean(config.relay?.deviceId) && (config.servers?.length || 0) > 0

  const steps = [
    {
      title: "Pair this device",
      detail: config.relay?.deviceId
        ? "This device is already bound."
        : "Paste a pairing link in Pair.",
      ...(config.relay?.deviceId
        ? { state: "done" as const }
        : {
            state: "action" as const,
            actionLabel: "Open Pair",
            onAction: handlers.onOpenPairing,
          }),
    },
    {
      title: "Add MCP",
      detail:
        (config.servers?.length || 0) > 0
          ? "At least one MCP is configured."
          : "Open MCP and add one server.",
      ...((config.servers?.length || 0) > 0
        ? { state: "done" as const }
        : {
            state: "action" as const,
            actionLabel: "Open MCP",
            onAction: handlers.onOpenServers,
          }),
    },
    {
      title: "Start relay",
      detail:
        status.state === "running"
          ? "Relay is serving traffic now."
          : "Start when setup is ready.",
      ...(status.state === "running"
        ? { state: "done" as const }
      ...
      <div className="grid items-start gap-8 xl:grid-cols-[1.15fr_0.85fr]">
        <section className="flex flex-col gap-5">
          <div>
            ...
            <div className="mt-2 text-sm text-muted-foreground">
              Start, stop, or restart the relay here.
            </div>
          </div>
        ...
        <section className="flex flex-col gap-4">
          <div>
            ...
            <div className="mt-2 text-sm text-muted-foreground">
              Complete the next missing step.
            </div>
          </div>
