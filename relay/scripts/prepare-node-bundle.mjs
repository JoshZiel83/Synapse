...
async function main() {
  ...
  try {
    ...
    console.log(
      `Downloading shared Node ${options.nodeVersion} for ${options.targetPlatform}`
    )
    ...
    const nodeBinary = options.targetPlatform.startsWith("windows-")
      ? "node/node.exe"
      : "node/bin/node"
    ...
    await mkdir(
      join(
        assetsDir,
        "node",
        options.targetPlatform.startsWith("windows-") ? "" : "bin"
      ),
      { recursive: true }
    )
    ...
    try {
      await copyFile(
        join(nodeSourceDir, "LICENSE"),
        join(assetsDir, "node", "LICENSE")
      )
    } catch {
    ...
    const manifest = {
      prepared: true,
      assetVersion: getSharedNodeAssetVersion(
