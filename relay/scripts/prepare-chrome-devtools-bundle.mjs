...
async function main() {
  ...
  try {
    ...
    await ensureExists(
      join(packageSourceDir, "build", "src", "bin", "chrome-devtools-mcp.js")
    )

    await rm(assetsDir, { recursive: true, force: true })
    await mkdir(assetsDir, { recursive: true })
    await cp(packageSourceDir, join(assetsDir, CHROME_DEVTOOLS_PACKAGE_DIR), {
      recursive: true,
    })

    const manifest = {
      prepared: true,
      assetVersion: getChromeDevtoolsAssetVersion(
        options,
