...
impl IndexStore {
    ...
    pub fn open_with_tika(
        ...
        //
        // v2 (round-9): bumped from 1 to purge any rows the buggy
        // pre-round-8 rebuild wrote with backslash filenames re-routed
        // into fake VFS prefixes (e.g. real `public\leak.txt` indexed as
        // `/public/leak.txt`). Those rows survive the rebuild fix
        // because the canonical() reject only stops *new* writes; old
...
///
/// Returns:
///   - `Ok(Some(conn))` — handle opened, ready for `compute_status`.
///   - `Ok(None)` — db file is genuinely absent (helper hasn't fully
///     initialized, or the file was deleted out from under us). The
///     status RPC treats this as "empty index" and returns the quiet
///     zero stub. This case is acceptable because the helper always
///     creates the db on startup; an absent file isn't an error worth
///     surfacing to callers, it's just "nothing to read".
///   - `Err(_)` — a real SQLite failure (corruption, permissions,
///     schema drift, etc.). The status RPC propagates these so the
///     caller sees `index_status_unavailable` instead of an incorrect
///     `doc_count:0` that pretends the index is healthy.
pub fn open_status_conn(work_dir: &Path) -> Result<Option<Connection>, RpcError> {
    let db_path = work_dir.join("index.sqlite");
    if !db_path.exists() {
        return Ok(None);
    }
