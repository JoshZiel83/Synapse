"""OpenTelemetry tracing for the mijia-mcp sidecar (logging refactor P7).
...
`traceparent` on its outbound HTTP to this sidecar (its HttpInstrumentation), so
StarletteInstrumentor auto-continues that trace and exports request spans to the
collector under one trace_id — making mijia part of the cross-process span tree.
...
def setup_tracing() -> bool:
    ...
    resource = Resource.create(
        {
            "service.name": os.environ.get("OTEL_SERVICE_NAME", "mijia-mcp"),
