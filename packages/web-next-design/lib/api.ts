...
// web-next-design: this is a UI-design clone with NO backend. When
// NEXT_PUBLIC_DESIGN_MOCK is on (default), `api` is a typed fake client that
// implements the exact ApiClient surface and serves data generated from the
// real @synapse/shared zod schemas (see lib/design/). Set the flag to "0" to
// restore the real network client. The fake wraps a real ApiClient instance in
// a Proxy, so it stays assignable to `ApiClient`; each mocked method is checked
// against this class's signatures in the handler modules at compile time.
import { createDesignApi } from "./design/mock-api"

const DESIGN_MOCK = process.env.NEXT_PUBLIC_DESIGN_MOCK !== "0"

export const api: ApiClient = DESIGN_MOCK ? createDesignApi() : new ApiClient()
