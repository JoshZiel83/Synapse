...
import { designServerAuthState } from "@/lib/design/auth"
...
export const getServerAuthState = cache(async (): Promise<ServerAuthState> => {
  // Design sandbox: no backend to validate against — hand back a stable fake
  // session so protected layouts render. Set NEXT_PUBLIC_DESIGN_MOCK=0 to use
  // the real /auth/me probe below.
  if (process.env.NEXT_PUBLIC_DESIGN_MOCK !== "0") {
    return designServerAuthState()
  }
