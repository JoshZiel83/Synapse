...
;(() => {
            ...
            Object.entries(snapshot.tombstones).filter(
              ([conversationId, entry]) =>
                Boolean(
                  ...
                  typeof entry === "object" &&
                  typeof entry.conversationId === "string" &&
                  typeof entry.removedSeq === "number"
                )
            )
          )
        : {}
  ...
  function dateToIsoInstant(value) {
    if (!isValidDateInstance(value)) {
      throw new Error(
        "Expected a valid Date when converting to IsoInstantString"
      )
    }
  ...
  function getQueueDatabase() {
    if (!queueDbPromise) {
      queueDbPromise = openDB(CHAT_QUEUE_DB_NAME, CHAT_QUEUE_DB_VERSION, {
        upgrade(database) {
  ...
  async function saveStoredChatQueueState(queueState) {
    ...
    await database.put(CHAT_QUEUE_STATE_STORE, {
      ...
      payload: normalizeStoredChatQueueState(
  ...
  function getWorkerDatabase() {
    if (!workerDbPromise) {
      workerDbPromise = openDB(CHAT_WORKER_DB_NAME, CHAT_WORKER_DB_VERSION, {
        upgrade(database) {
          if (
  ...
  async function broadcast(message) {
    try {
      if ("BroadcastChannel" in scope) {
        const channel = new BroadcastChannel(CHAT_QUEUE_BROADCAST_CHANNEL)
  ...
  async function runSyncPass(workspaceIdOverride, reason) {
    try {
      ...
      let nextSnapshot = await flushPendingReads(
        effectiveAuth,
        startingSnapshot
      )
