        ...
        (conversation): conversation is ChatConversationView =>
          Boolean(
            conversation &&
              ...
              typeof (conversation as { conversationId?: unknown }).conversationId === "string" &&
              typeof (conversation as { workspaceId?: unknown }).workspaceId === "string" &&
              typeof (conversation as { title?: unknown }).title === "string" &&
              (conversation as { presentation?: unknown }).presentation &&
              typeof (conversation as { presentation?: unknown }).presentation === "object" &&
              Array.isArray((conversation as { participants?: unknown }).participants),
          ),
      )
    : [];
  ...
  return {
    ...
    itemsByConversationId:
      ...
      typeof snapshot.itemsByConversationId === "object"
        ? Object.fromEntries(
            Object.entries(
              snapshot.itemsByConversationId as Record<string, ChatConversationItem[]>,
...
export function sortChatConversations(conversations: ChatConversationView[]) {
  return [...conversations].sort((left, right) => {
    ...
    const rightAt = right.lastItem?.createdAt ?? right.updatedAt ?? right.createdAt;
    return new Date(rightAt).getTime() - new Date(leftAt).getTime();
  });
}

export function sortChatItems<T extends Pick<ChatConversationItem, "sequence" | "createdAt">>(
  items: T[],
) {
...
export function getParticipantDisplayName(
  participant: Pick<ChatParticipantSummary, "name" | "participantType"> | undefined,
) {
  const displayName =
...
export function buildPreviewTextFromItem(item: ChatConversationItem | undefined) {
  if (!item) {
    return "";
  }
...
export function getMobileConversationItems(
  ...
  const confirmedItems = (snapshot.itemsByConversationId[conversationId] ?? []).map<
    MobileChatItem
  >((item) => item);
  const optimisticItems = Object.values(snapshot.outbox)
    ...
    .filter(
      (entry) =>
        !confirmedItems.some(
          (item) =>
            item.clientMessageId && item.clientMessageId === entry.clientMessageId,
        ),
    )
    .map((entry) =>
