...
function targetLabel(
  context: AutomationEventSourceDefinitionContext | AutomationOccurrenceDisplayContext,
  fallback: string,
) {
  return (
    readString((context as AutomationEventSourceDefinitionContext).integrationTargetLabel) ||
    readString((context as AutomationEventSourceDefinitionContext).providerLabel) ||
    readString((context as AutomationEventSourceDefinitionContext).providerRef) ||
...
function githubRepositoryLabel(context: AutomationOccurrenceDisplayContext) {
  return (
    readString(
      (context.payload.repository as Record<string, unknown> | undefined)?.full_name,
    ) ||
...
function gitlabProjectLabel(context: AutomationOccurrenceDisplayContext) {
  return (
    readString(
      (context.payload.project as Record<string, unknown> | undefined)?.path_with_namespace,
    ) ||
    readString(context.sourceSnapshot.integrationTargetLabel) ||
...
function buildGithubIssueCommentDisplay(context: AutomationOccurrenceDisplayContext) {
  const repo = githubRepositoryLabel(context);
  const issueNumber = readString(
    (context.payload.issue as Record<string, unknown> | undefined)?.number,
  ) || String(
    (context.payload.issue as Record<string, unknown> | undefined)?.number || "",
  ).trim() || "issue";
  const author = readString(
    ((context.payload.comment as Record<string, unknown> | undefined)?.user as Record<string, unknown> | undefined)?.login,
  );
...
function buildGithubPullRequestDisplay(context: AutomationOccurrenceDisplayContext) {
  const repo = githubRepositoryLabel(context);
  const prNumber = String(
    (context.payload.pull_request as Record<string, unknown> | undefined)?.number || "",
  ).trim() || "pull request";
...
function buildGithubPullRequestReviewDisplay(context: AutomationOccurrenceDisplayContext) {
  const repo = githubRepositoryLabel(context);
  const prNumber = String(
    (context.payload.pull_request as Record<string, unknown> | undefined)?.number || "",
  ).trim() || "pull request";
  ...
  const author = readString(
    ((context.payload.review as Record<string, unknown> | undefined)?.user as Record<string, unknown> | undefined)?.login,
  );
...
function buildGithubWorkflowRunDisplay(context: AutomationOccurrenceDisplayContext) {
  const repo = githubRepositoryLabel(context);
  const workflowName = readString(
    (context.payload.workflow as Record<string, unknown> | undefined)?.name,
  ) || readString(
    (context.payload.workflow_run as Record<string, unknown> | undefined)?.name,
  ) || "workflow";
  const status = readString(
    (context.payload.workflow_run as Record<string, unknown> | undefined)?.conclusion,
  ) || readString(
    (context.payload.workflow_run as Record<string, unknown> | undefined)?.status,
  ) || "updated";
...
function buildGitlabNoteDisplay(context: AutomationOccurrenceDisplayContext) {
  ...
  const note = readString(
    (context.payload.object_attributes as Record<string, unknown> | undefined)?.note,
  );
  const noteableType = readString(
    (context.payload.object_attributes as Record<string, unknown> | undefined)?.noteable_type,
  ) || "item";
  return {
    title: `${project} note`,
...
function buildGitlabMergeRequestDisplay(context: AutomationOccurrenceDisplayContext) {
  const project = gitlabProjectLabel(context);
  const title = readString(
    (context.payload.object_attributes as Record<string, unknown> | undefined)?.title,
  );
  const action = readString(
    (context.payload.object_attributes as Record<string, unknown> | undefined)?.action,
  ) || "updated";
  return {
...
function buildGitlabPipelineDisplay(context: AutomationOccurrenceDisplayContext) {
  const project = gitlabProjectLabel(context);
  const status = readString(
    (context.payload.object_attributes as Record<string, unknown> | undefined)?.status,
  ) || "updated";
  const ref = readString(
    (context.payload.object_attributes as Record<string, unknown> | undefined)?.ref,
  ) || "unknown";
  return {
...
function buildGitlabPushDisplay(context: AutomationOccurrenceDisplayContext) {
  ...
  const commits = typeof context.payload.total_commits_count === "number"
...
export const githubIssueCommentEventDefinition: AutomationEventDefinition = {
  ...githubSource(
    ...
    (target) => `Triggered when a new issue or pull request comment is created in ${target}.`,
    (target) => `Use this for repo inbox workflows in ${target}, such as waking reviewers or triaging inbound comments.`,
    {
      type: "object",
      properties: {
        action: { type: "string" },
...
export const githubPullRequestEventDefinition: AutomationEventDefinition = {
  ...githubSource(
    ...
    (target) => `Use this for review orchestration in ${target}, such as reacting to newly opened, synchronized, or merged pull requests.`,
    {
      type: "object",
      properties: {
        action: { type: "string" },
...
export const githubPullRequestReviewEventDefinition: AutomationEventDefinition = {
  ...githubSource(
    ...
    (target) => `Triggered when a pull request review is submitted in ${target}.`,
    (target) => `Use this when review state changes in ${target} should wake agents or notify humans immediately.`,
    {
      type: "object",
      properties: {
        action: { type: "string" },
...
export const githubPushEventDefinition: AutomationEventDefinition = {
  ...githubSource(
    ...
    (target) => `Use this for branch-level automation in ${target}, such as post-push indexing or sync notifications.`,
    {
      type: "object",
      properties: {
        ref: { type: "string" },
...
export const gitlabNoteEventDefinition: AutomationEventDefinition = {
  ...gitlabSource(
    "gitlab.note",
    "GitLab Note",
    (target) => `Triggered when a new note or discussion reply is created in ${target}.`,
    (target) => `Use this for comment-driven workflows in ${target}, such as triage, escalation, or reviewer wakeups.`,
    {
      type: "object",
      properties: {
...
export const gitlabPipelineEventDefinition: AutomationEventDefinition = {
  ...gitlabSource(
    ...
    (target) => `Use this for CI/CD orchestration in ${target}, especially when pipeline status should wake sessions or notify operators.`,
    {
      type: "object",
      properties: {
        object_kind: { type: "string" },
