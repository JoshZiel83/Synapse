...
// `browserPolicyAllows`. Schema is `.strip()` (zod default) — unknown keys
// like a stray `scopeSource` get silently dropped on parse. We do NOT use
// `.strict()` because the approval path may copy the requested action's
// `scopeSource` into a default grant option; stripping keeps that flow
// from blowing up.
