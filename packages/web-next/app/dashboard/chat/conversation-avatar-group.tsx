...
function moveActiveActorToFront(
  ...
  const activeIndex = participants.findIndex(
    (participant) => participant.id === activeActorId
  )
...
export default function ConversationAvatarGroup({
  ...
  const orderedParticipants = moveActiveActorToFront(
    participants,
    activeActorId
  )
      ...
      <AvatarGroup className={className}>
        <Avatar size={size}>
          ...
          <AvatarBadge className="size-4 bg-background p-0.5 shadow-sm ring-1 ring-border">
            <img
              src={getTwemojiUrl(STATUS_EMOJI.idle) || undefined}
              alt="Idle"
              className="size-full"
            />
          </AvatarBadge>
  ...
  return (
    <AvatarGroup className={cn("items-center", className)}>
      {visibleParticipants.map((participant) => {
        ...
        const phase =
          participant.id === activeActorId ? activePhase || "thinking" : "idle"
