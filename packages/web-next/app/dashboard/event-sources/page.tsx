...
export default function EventSourcesPage() {
  ...
  const [statusFilter, setStatusFilter] = useState<"all" | EventSourceStatus>("all");
  ...
  async function loadOccurrences(eventSourceId: string) {
    ...
    try {
      const nextOccurrences = await api.getAutomationEventSourceOccurrences(workspaceId, eventSourceId);
  ...
  const selectedSource = useMemo(
    () => filteredSources.find((source) => source.id === selectedSourceId) || sources.find((source) => source.id === selectedSourceId) || null,
  ...
  async function handleCreateSource() {
    ...
    try {
      await api.createAutomationEventSource(workspaceId, {
        ...
        payloadSchema: parseJsonRecord(formState.payloadSchemaText, "payload schema"),
        examplePayload: parseJsonRecord(formState.examplePayloadText, "example payload"),
      });
      ...
      toast.error(error instanceof Error ? error.message : "Failed to create event source");
    } finally {
      setSavingSource(false);
    }
  }

  async function handleUpdateStatus(source: AutomationEventSource, status: EventSourceStatus) {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Failed to update event source");
    } finally {
      setSavingSource(false);
    }
  }
  ...
  async function handleArchiveSource(source: AutomationEventSource) {
    if (!workspaceId) return;
    if (!window.confirm(`Archive event source "${source.name}"? Existing subscriptions will be paused.`)) {
      return;
    }
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Failed to archive event source");
    } finally {
      setSavingSource(false);
    }
  }
  ...
  return (
    <div className="flex flex-col gap-6 px-4 pb-6 pt-6 lg:px-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-foreground">Event Sources</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            Register durable event definitions for {workspaceName || "this workspace"}, inspect their lifecycle,
            and review every reported occurrence.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Button type="button" variant="outline" onClick={() => void loadSources()} disabled={loadingSources}>
            <RefreshCw className={cn(loadingSources && "animate-spin")} data-icon="inline-start" />
            Refresh
          </Button>
          ...
          <AppCardHeader className="gap-4">
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <AppCardDescription>
                  Active, deprecated, disabled, and archived sources stay discoverable for auditability.
                </AppCardDescription>
              </div>
            ...
            <div className="grid gap-3">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <Select value={statusFilter} onValueChange={(value) => setStatusFilter(value as "all" | EventSourceStatus)}>
                ...
                <Select value={providerFilter} onValueChange={(value) => setProviderFilter(value as "all" | EventSourceProvider)}>
                  <SelectTrigger className="w-full">
              ...
              filteredSources.map((source) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p className="mt-3 line-clamp-3 text-sm text-muted-foreground">{source.description}</p>
                    {source.recommendedUsage ? (
              ...
              <AppCard variant="panel">
                  ...
                  <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <Badge variant={eventSourceStatusVariant(selectedSource.status)}>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selectedSource.status !== "active" ? (
                        <Button
                          ...
                          variant="outline"
                          size="sm"
                          onClick={() => void handleUpdateStatus(selectedSource, "active")}
                      ...
                      {selectedSource.status !== "deprecated" ? (
                        <Button
                          ...
                          variant="outline"
                          size="sm"
                          onClick={() => void handleUpdateStatus(selectedSource, "deprecated")}
                          disabled={savingSource}
                        >
                          Deprecate
                        </Button>
                      ...
                      {selectedSource.status !== "disabled" ? (
                        <Button
                          ...
                          variant="outline"
                          size="sm"
                          onClick={() => void handleUpdateStatus(selectedSource, "disabled")}
                          disabled={savingSource}
                        >
                          Disable
                        </Button>
                      ...
                      {selectedSource.status !== "archived" ? (
                        <Button
                          ...
                          variant="destructive"
                          size="sm"
                          onClick={() => void handleArchiveSource(selectedSource)}
                          disabled={savingSource}
                        >
                ...
                <AppCardContent className="grid gap-6 xl:grid-cols-3">
                  <div className="rounded-[22px] border border-border/70 bg-muted/20 p-4">
                    <div className="text-sm font-medium text-foreground">Identity</div>
                    <dl className="mt-3 grid gap-3 text-sm">
                      ...
                      <div>
                        ...
                        <dd className="font-medium text-foreground">{formatDateTime(selectedSource.createdAt)}</dd>
                      </div>
                      <div>
                        <dt className="text-muted-foreground">Last triggered</dt>
                        <dd className="font-medium text-foreground">{formatDateTime(selectedSource.lastTriggeredAt)}</dd>
                      </div>
                    </dl>
                  </div>
                  <div className="rounded-[22px] border border-border/70 bg-muted/20 p-4">
                    <div className="text-sm font-medium text-foreground">Suggested Usage</div>
                    <p className="mt-3 whitespace-pre-wrap text-sm text-muted-foreground">
                      {selectedSource.recommendedUsage || "No suggested usage guidance provided for this source."}
                    </p>
                  </div>
                  <div className="rounded-[22px] border border-border/70 bg-muted/20 p-4">
                    ...
                    <div className="mt-3 grid gap-4">
                      <div>
                        <div className="text-xs uppercase tracking-wide text-muted-foreground">Payload schema</div>
                        <pre className="mt-2 overflow-x-auto rounded-2xl bg-background px-3 py-3 text-xs">
                          {serializeJson(selectedSource.payloadSchema)}
                        </pre>
                      </div>
                      <div>
                        <div className="text-xs uppercase tracking-wide text-muted-foreground">Example payload</div>
                        <pre className="mt-2 overflow-x-auto rounded-2xl bg-background px-3 py-3 text-xs">
                ...
                <AppCardHeader className="gap-3">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <AppCardDescription>
                        Every reported occurrence is stored even if no trigger subscribed to it.
                      </AppCardDescription>
                    </div>
                    <Button
                      type="button"
                    ...
                    >
                      <RefreshCw className={cn(loadingOccurrences && "animate-spin")} data-icon="inline-start" />
                      Refresh
                    </Button>
                  </div>
                </AppCardHeader>
                <AppCardContent className="flex flex-col gap-4">
                  {loadingOccurrences ? (
                    ...
                    occurrences.map((occurrence, index) => (
                      <div key={occurrence.id} className="rounded-[22px] border border-border/70 px-4 py-4">
                        <div className="flex flex-wrap items-start justify-between gap-3">
                          <div className="min-w-0">
                            <div className="flex items-center gap-2 text-sm font-medium text-foreground">
                              <History className="size-4 text-muted-foreground" />
                              <span className="truncate">{occurrenceTitle(occurrence)}</span>
                            </div>
                            <div className="mt-1 text-xs text-muted-foreground">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {occurrence.dedupeKey ? <Badge variant="outline">dedupe {occurrence.dedupeKey}</Badge> : null}
                            <Badge variant="secondary">{occurrenceSummary(occurrence)}</Badge>
                          </div>
                        </div>
                        ...
                        {index < occurrences.length - 1 ? <Separator className="mt-4" /> : null}
                      </div>
                    ))
                  )}
                </AppCardContent>
          ...
          <div className="grid gap-4">
            <div className="grid gap-4 md:grid-cols-2">
              <div className="grid gap-2">
                <label className="text-sm font-medium text-foreground" htmlFor="event-source-provider-kind">
                  Provider
                </label>
                <Select
            ...
            <div className="rounded-[22px] border border-border/70 bg-muted/20 px-4 py-3 text-sm text-muted-foreground">
              ...
              can be enabled without creating duplicates.
            </div>
          </div>
          <DialogFooter>
            <Button type="button" variant="outline" onClick={() => setCreateDialogOpen(false)} disabled={savingSource}>
              Cancel
            </Button>
            <Button type="button" onClick={() => void handleCreateSource()} disabled={savingSource}>
              {savingSource ? (
