...
export default function DashboardPage() {
  ...
  const [stats, setStats] = useState<Stats>({ contacts: 0, activeWorkItems: 0, memories: 0 });
  ...
  async function loadStats() {
    setLoading(true);
    try {
      const [members, actors, workItems, memories] = await Promise.allSettled([
      ...
      setStats({
        contacts:
          (members.status === 'fulfilled'
            ? (Array.isArray(members.value) ? members.value.length : (members.value?.data?.length || 0))
            : 0) +
          (actors.status === 'fulfilled' ? (Array.isArray(actors.value) ? actors.value.length : 0) : 0),
  ...
  const quickActions = [
    ...
    },
    {
      ...
      description: 'View workspace users and actors in one place',
      icon: ContactRound,
      color: 'from-violet-500 to-purple-500',
      href: '/dashboard/contacts',
    },
    {
      title: 'Open Chat',
      description: 'Jump into your group conversations and direct collaboration flows',
      icon: MessageSquare,
      color: 'from-emerald-500 to-teal-500',
      href: '/dashboard/chat',
    },
