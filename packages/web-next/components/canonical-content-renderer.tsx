...
function FilePreview({ file }: { file: FileRefBlock }) {
  ...
  if (file.category === "image") {
    return (
      ...
      >
        {fileUrl ? (
          <img src={fileUrl} alt={file.originalName} className="aspect-[16/10] w-full object-cover" />
        ) : (
        ...
        <div className="flex items-center justify-between gap-3 border-t border-border px-4 py-3">
          <div className="min-w-0">
            <div className="truncate text-sm font-medium text-foreground">{file.originalName}</div>
  ...
  if (file.category === "video") {
    return (
      <div className="overflow-hidden rounded-3xl border border-border bg-muted/30">
        {fileUrl ? (
          <video controls src={fileUrl} className="aspect-video w-full bg-black/90" />
        ) : (
    ...
    <div className={cn("flex flex-col gap-5", className)}>
      {blocks.map((block) =>
        block.type === "text" ? (
          <div
            key={block.id}
            className="whitespace-pre-wrap rounded-3xl border border-border bg-background/70 px-4 py-3 text-sm leading-7 text-foreground/85"
          >
            {block.text || <span className="text-muted-foreground">Empty text block</span>}
          </div>
        ) : (
