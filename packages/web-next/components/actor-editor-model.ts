...
export const ACTOR_DOC_GROUPS = [
  {
    value: "identity",
    label: "Identity",
    keys: ["identity_card", "public_persona", "soul", "self_narrative", "origin_story"] satisfies CoreActorDocKey[],
  },
  {
    value: "social",
    label: "Social",
  ...
  },
  {
    value: "work",
    label: "Work",
    keys: ["role_charter", "mission", "work_doctrine", "limitations_and_escalation", "routines"] satisfies CoreActorDocKey[],
  },
  {
    value: "voice",
    label: "Voice",
    keys: ["quirks_and_signatures", "conversation_examples"] satisfies CoreActorDocKey[],
  },
] as const

export const VISIBILITY_OPTIONS: Array<{ value: ActorDocVisibility; label: string }> = [
...
function getDocTemplate(key: ActorDocKey) {
  return key === "custom" ? undefined : ACTOR_DOC_TEMPLATES.find((item) => item.key === key)
}
...
export function summarizeBlocks(blocks: CanonicalContentBlock[], maxLength = 200) {
  const text = extractText(blocks).replace(/\s+/g, " ").trim()
  if (text) {
    return text.length > maxLength ? `${text.slice(0, maxLength - 1)}...` : text
  }
  const file = blocks.find((block): block is FileRefBlock => block.type === "file_ref")
...
export function actorSummary(actor: Actor) {
  const docs = [...(actor.definition.docs || [])].sort((left, right) => right.priority - left.priority)
...
export function buildEditableDocFromTemplate(templateKey: CoreActorDocKey): EditableDoc {
...
export function buildEditableCustomDoc(): EditableDoc {
  return {
    ...
    description: "Additional profile context that does not fit the common sections.",
    visibility: "always",
    priority: 40,
    content: [createLocalTextBlock("")],
  }
...
export function getAvailableActorDocTemplates(docs: Array<Pick<EditableDoc, "key">>) {
  const existingStandardKeys = new Set(
    docs.filter((doc) => !isCustomDocKey(doc.key)).map((doc) => doc.key as CoreActorDocKey),
  )
  return ACTOR_DOC_TEMPLATES.filter((template) => !existingStandardKeys.has(template.key))
}
