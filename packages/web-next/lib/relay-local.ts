...
export async function sendPairingToLocalRelayDesktop(input: {
  ...
  if (!response.ok || !payload.accepted) {
    throw new Error(payload.message || 'Desktop relay client rejected the pairing request');
  }

  return payload;
}

export function buildRelayDesktopDeepLink(pairing: RelayPairingSessionView): string {
