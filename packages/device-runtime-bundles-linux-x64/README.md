# @synapse/device-runtime-bundles-linux-x64

Pre-staged toolchain archives for the `linux-x64` platformKey, consumed
...
`npm install @synapse/device-runtime` on a `linux-x64` host fetches THIS
package's tarball and skips the other five.

The toolchain manager then resolves bundled toolchains from
`node_modules/@synapse/device-runtime-bundles-linux-x64/bundles/` BEFORE
