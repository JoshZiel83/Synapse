# @synapse/device-runtime-bundles-darwin-x64

Pre-staged toolchain archives for the `darwin-x64` platformKey, consumed
...
`npm install @synapse/device-runtime` on a `darwin-x64` host fetches THIS
package's tarball and skips the other five.

The toolchain manager then resolves bundled toolchains from
`node_modules/@synapse/device-runtime-bundles-darwin-x64/bundles/` BEFORE
