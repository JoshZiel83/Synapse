...
export const CURRENT_SCHEMA_VERSION = "2026-06-09-datetime-unification"
export const CURRENT_SCHEMA_DESCRIPTION =
  "Datetime boundary unification: all persisted instants remain TIMESTAMPTZ-backed Dates in generated/db.ts, shared protocol timestamps are branded IsoInstantString values, and schema bootstrap now installs a generic updated_at auto-touch trigger across every public base table that exposes updated_at. Requires db:rebuild for existing databases (no in-place migrations during initial design). " +
