...
export const CURRENT_SCHEMA_VERSION = "2026-07-14-sandbox-r6-recovering-lease"
export const CURRENT_SCHEMA_DESCRIPTION =
  "Sandbox review R6 (H-2): file_mount_status gains a 'recovering' value — a durable LEASE a failed-mount recovery sweep CASes 'failed'→'recovering' (FOR UPDATE SKIP LOCKED) so only one worker/replica re-pulls a given VM; a crashed worker's stale lease is re-claimed once updated_at ages past the TTL. 'recovering' is NOT a live mount (excluded from uq_file_mounts_active_session_space/_subpath + idx_file_mounts_live_sandbox alongside 'closed'/'failed', so a failed→recovering CAS never collides with a re-provisioned generation's live mount) but IS still failed-recoverable (keepalive + the #4 data-free defer keep its sole-source VM alive). Requires db:rebuild for existing databases (no in-place migrations during initial design). " +
