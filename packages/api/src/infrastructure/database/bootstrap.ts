...
export const CURRENT_SCHEMA_VERSION = "2026-07-14-sandbox-r6-cleanup"
export const CURRENT_SCHEMA_DESCRIPTION =
  "Sandbox review R6: file_mount_status gains a 'recovering' value — a durable LEASE a failed-mount recovery sweep CASes 'failed'→'recovering' (FOR UPDATE SKIP LOCKED) so only one worker/replica re-pulls a given VM; a crashed worker's stale lease is re-claimed once updated_at ages past the TTL. 'recovering' is NOT a live mount (excluded from uq_file_mounts_active_session_space/_subpath + idx_file_mounts_live_sandbox alongside 'closed'/'failed', so a failed→recovering CAS never collides with a re-provisioned generation's live mount) but IS still failed-recoverable (keepalive + the #4 data-free defer keep its sole-source VM alive). Clean-slate: the never-wired sandboxes.stash_manifest_id column is dropped. Requires db:rebuild for existing databases (no in-place migrations during initial design). " +
  "Sandbox review R3: the sandbox mode<->service_kind constraint trigger is rewritten to be snapshot-independent (re-reads the final runtime kind + sandbox mode/state + all services by runtime_id at deferred execution), now fires on runtime_services DELETE (closing the detach-last-service hole) and sandboxes.state, and requires EXACTLY ONE correct-kind service when state='active' (a resident sandbox with zero services, a wrong-kind service, and device_runtime+bare_dataplane all now fail). sandboxes.platform/arch are now NOT NULL (fail-closed, no back-compat); sandboxes gains a nullable host_pid_identity column (boot_id:starttime, for PID-reuse-safe local recovery) and an idx_sandboxes_reap partial index on (state, deadline_at) WHERE state='provisioning'. Requires db:rebuild for existing databases (no in-place migrations during initial design). " +
