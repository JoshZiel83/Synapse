...
function sendServiceError(reply: FastifyReply, error: unknown) {
  ...
  if (/not allowed|forbidden|access denied|authentication failed/i.test(message)) {
...
export default async function remoteAgentsController(app: FastifyInstance) {
  ...
  }>("/api/v1/workspaces/:workspaceId/remote-agents", { preHandler: workspacePreHandler }, async (request, reply) => {
    try {
      return reply.send(
  ...
  }>("/api/v1/workspaces/:workspaceId/remote-agents/:remoteAgentId", { preHandler: workspacePreHandler }, async (request, reply) => {
    try {
      return reply.send(
  ...
  app.post<{
    Params: { workspaceId: string; remoteAgentId: string };
    Body: unknown;
  }>("/api/v1/workspaces/:workspaceId/remote-agents/:remoteAgentId/bind", { preHandler: workspacePreHandler }, async (request, reply) => {
    if (!(await requireWorkspaceRemoteAgentAdmin(request, reply))) return;
    try {
  ...
  app.post<{
    Params: { workspaceId: string };
    Body: unknown;
  }>("/api/v1/workspaces/:workspaceId/remote-agent-machines/pairing-sessions", { preHandler: workspacePreHandler }, async (request, reply) => {
    if (!(await requireWorkspaceRemoteAgentAdmin(request, reply))) return;
    try {
  ...
  app.get<{
    Params: { workspaceId: string };
  }>("/api/v1/workspaces/:workspaceId/remote-agent-machines", { preHandler: workspacePreHandler }, async (request, reply) => {
    try {
      return reply.send(
  ...
  }>("/api/v1/workspaces/:workspaceId/remote-agent-machines/:machineId", { preHandler: workspacePreHandler }, async (request, reply) => {
    try {
      return reply.send(
  ...
  for (const prefix of internalPrefixes) {
    ...
    app.get<{
      Params: { remoteAgentId: string };
      Querystring: unknown;
    }>(`${prefix}/remote-agents/:remoteAgentId/check-messages`, async (request, reply) => {
      try {
    ...
    app.get<{
      ...
      Querystring: unknown;
    }>(`${prefix}/remote-agents/:remoteAgentId/history/:conversationId`, async (request, reply) => {
      try {
    ...
    app.post<{
      Params: { remoteAgentId: string };
      Body: unknown;
    }>(`${prefix}/remote-agents/:remoteAgentId/send`, async (request, reply) => {
      try {
    ...
    app.get<{
      Params: { remoteAgentId: string };
      Querystring: unknown;
    }>(`${prefix}/remote-agents/:remoteAgentId/search`, async (request, reply) => {
      try {
