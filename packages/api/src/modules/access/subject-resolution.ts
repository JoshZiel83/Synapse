...
export async function buildConversationCapabilitySubjects(params: {
  ...
  if (!context && params.actorId && params.conversationId) {
    context = await getConversationActorContextByPair(
      params.conversationId,
      params.actorId,
    ) || undefined;
  }
  ...
  if (!context && params.sessionId) {
    context = await getConversationActorContextBySessionId(
      params.sessionId,
    ) || undefined;
  }
  ...
  if (
    ...
    (!params.conversationId || context.conversation_id === params.conversationId) &&
