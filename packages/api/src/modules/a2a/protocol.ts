import { isGroupConversationKind, type SessionStatus, type A2ATaskState, type A2AMessage, type A2ATaskResponse, type A2APart } from '@synapse/shared';
...
export async function buildTaskResponse(
  ...
  if (session.status === 'idle' && !isGroupConversationKind(session.conversation_kind)) {
    state = 'completed';
  }
