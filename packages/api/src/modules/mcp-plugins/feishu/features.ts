import type { PluginConfigFieldOption } from "@synapse/shared"
...
export const FEISHU_FEATURES = [
  {
    ...
    scopes: ["contact:user:search", "contact:user.basic_profile:readonly"],
  },
  {
    key: "im_read",
  ...
  },
  {
    ...
    scopes: ["search:message", "contact:user.basic_profile:readonly"],
  ...
  },
  {
    ...
    scopes: ["sheets:spreadsheet:read", "sheets:spreadsheet:write_only"],
  },
  {
    key: "base",
...
const featureSet = new Set<string>(
  FEISHU_FEATURES.map((feature) => feature.key)
)
...
export function normalizeFeishuFeatureKeys(value: unknown): FeishuFeatureKey[] {
  if (Array.isArray(value)) {
    return Array.from(new Set(value.filter(isFeishuFeatureKey)))
  }
...
export function assertFeishuFeatureSelection(features: FeishuFeatureKey[]) {
  if (features.length === 0) {
    throw new Error(
      "Select at least one Feishu feature before connecting the account."
    )
  }
}
...
export function getFeishuFeatureScopeCoverage(
  ...
  return features.map((featureKey) => {
    ...
    return {
      ...
      mayRequireAppReview: Boolean(
        feature &&
        "mayRequireAppReview" in feature &&
        feature.mayRequireAppReview === true
      ),
      scopes,
