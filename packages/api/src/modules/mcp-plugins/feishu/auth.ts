...
async function beginAppRegistration() {
  ...
  return {
    ...
    expiresAt: new Date(Date.now() + asNumber(payload.expires_in, 300) * 1000).toISOString(),
    verificationUrl: buildVerificationUrl(resolveFeishuOpenBaseUrl("feishu"), userCode),
    userCode,
  };
}

async function pollAppRegistration(
...
> {
  ...
  if (!error && asString(result.client_id)) {
    ...
    if (!asString(result.client_secret) && tenantBrand === "lark" && payload.brand !== "lark") {
...
async function beginUserAuthorization(input: {
  ...
  const basicAuth = Buffer.from(`${input.appId}:${input.appSecret}`).toString("base64");
  ...
  return {
    ...
    deviceCode: asString(payload.device_code),
    interval: asNumber(payload.interval, 5),
    expiresAt: new Date(Date.now() + asNumber(payload.expires_in, 240) * 1000).toISOString(),
    verificationUrl,
