...
function normalizeCoordinatePair(value: string, label: string) {
  ...
  if (!Number.isFinite(lng) || !Number.isFinite(lat)) {
    throw new Error(`${label} must contain valid longitude and latitude numbers.`);
  }
  return `${lng},${lat}`;
}
...
function normalizeCoordinateSequence(
  ...
  if (pairs.length < minPairs) {
    throw new Error(`${label} must include at least ${minPairs} coordinate pair(s).`);
  }
...
function buildCommonQuery(config: JsonObject) {
  ...
  if (!apiKey) {
    throw new Error("AMap credentials not configured. Set apiKey in the plugin config.");
  }
  ...
  const sig = readOptionalString(config.sig) || readOptionalString(config.signature);
  if (sig) {
    query.set("sig", sig);
  }
...
async function fetchAmap(
  ...
  try {
    ...
    if (
      ...
      const message = firstNonEmpty(parsed.info, parsed.errmsg, parsed.message) || "Unknown error";
      throw new AmapApiError(
...
function renderGeocodeItem(item: JsonObject, index: number) {
  ...
  return formatBlock(
    ...
    [
      ...
      firstNonEmpty(item.location) && `location: ${firstNonEmpty(item.location)}`,
      firstNonEmpty(item.adcode) && `adcode: ${firstNonEmpty(item.adcode)}`,
      firstNonEmpty(item.level) && `level: ${firstNonEmpty(item.level)}`,
      firstNonEmpty(neighborhood.name) && `neighborhood: ${firstNonEmpty(neighborhood.name)}`,
      firstNonEmpty(building.name) && `building: ${firstNonEmpty(building.name)}`,
    ],
  );
}
...
function renderPoiItem(item: JsonObject, index: number) {
  ...
  return formatBlock(
    ...
    [
      ...
      firstNonEmpty(item.location) && `location: ${firstNonEmpty(item.location)}`,
      firstNonEmpty(item.type, item.typecode) && `type: ${firstNonEmpty(item.type, item.typecode)}`,
      firstNonEmpty(item.tel) && `tel: ${firstNonEmpty(item.tel)}`,
      formatDistance(item.distance) && `distance: ${formatDistance(item.distance)}`,
      firstNonEmpty(item.opentime, business.opentime_today) && `hours: ${firstNonEmpty(item.opentime, business.opentime_today)}`,
      firstNonEmpty(bizExt.rating, business.rating) && `rating: ${firstNonEmpty(bizExt.rating, business.rating)}`,
      firstNonEmpty(item.biz_type) && `business type: ${firstNonEmpty(item.biz_type)}`,
...
function renderTipItem(item: JsonObject, index: number) {
  return formatBlock(
    ...
    [
      firstNonEmpty(item.id) && `id: ${firstNonEmpty(item.id)}`,
      joinNonEmpty([firstNonEmpty(item.district), firstNonEmpty(item.address)], " / ")
      && `address: ${joinNonEmpty([firstNonEmpty(item.district), firstNonEmpty(item.address)], " / ")}`,
      firstNonEmpty(item.location) && `location: ${firstNonEmpty(item.location)}`,
      firstNonEmpty(item.typecode) && `typecode: ${firstNonEmpty(item.typecode)}`,
    ],
  );
}
...
function renderDistrictItem(item: JsonObject, index: number) {
  ...
  return formatBlock(
    ...
    [
      firstNonEmpty(item.level) && `level: ${firstNonEmpty(item.level)}`,
      firstNonEmpty(item.adcode) && `adcode: ${firstNonEmpty(item.adcode)}`,
      firstNonEmpty(item.citycode) && `citycode: ${firstNonEmpty(item.citycode)}`,
...
function renderRoadItem(item: JsonObject, index: number) {
  return formatBlock(
    ...
    [
      firstNonEmpty(item.location) && `location: ${firstNonEmpty(item.location)}`,
      formatDistance(item.distance) && `distance: ${formatDistance(item.distance)}`,
      firstNonEmpty(item.direction) && `direction: ${firstNonEmpty(item.direction)}`,
    ],
  );
}
...
function renderRouteStep(item: JsonObject, index: number) {
  ...
  return formatBlock(
    ...
    [
      ...
      formatDistance(item.step_distance ?? item.distance) && `distance: ${formatDistance(item.step_distance ?? item.distance)}`,
      formatDurationSeconds(item.duration) && `duration: ${formatDurationSeconds(item.duration)}`,
      firstNonEmpty(item.orientation) && `orientation: ${firstNonEmpty(item.orientation)}`,
    ],
  );
}
...
function formatReverseGeocodeResponse(result: JsonObject) {
  ...
  const sections = [
    formatBlock("Reverse geocoding result", [
      firstNonEmpty(regeo.formatted_address) && `address: ${firstNonEmpty(regeo.formatted_address)}`,
      joinNonEmpty(
        [
      ...
      firstNonEmpty(component.adcode) && `adcode: ${firstNonEmpty(component.adcode)}`,
      firstNonEmpty(component.citycode) && `citycode: ${firstNonEmpty(component.citycode)}`,
      firstNonEmpty(neighborhood.name) && `neighborhood: ${firstNonEmpty(neighborhood.name)}`,
      firstNonEmpty(building.name) && `building: ${firstNonEmpty(building.name)}`,
    ]),
  ];
...
function formatWeatherResult(result: JsonObject) {
  ...
  if (Object.keys(live).length > 0) {
    return formatBlock(
      ...
      [
        joinNonEmpty([firstNonEmpty(live.province), firstNonEmpty(live.city)], " / ")
        && `area: ${joinNonEmpty([firstNonEmpty(live.province), firstNonEmpty(live.city)], " / ")}`,
        firstNonEmpty(live.weather) && `weather: ${firstNonEmpty(live.weather)}`,
        firstNonEmpty(live.temperature) && `temperature: ${firstNonEmpty(live.temperature)} C`,
        firstNonEmpty(live.humidity) && `humidity: ${firstNonEmpty(live.humidity)}`,
        joinNonEmpty([firstNonEmpty(live.winddirection), firstNonEmpty(live.windpower)], " ")
        && `wind: ${joinNonEmpty([firstNonEmpty(live.winddirection), firstNonEmpty(live.windpower)], " ")}`,
        firstNonEmpty(live.reporttime) && `report time: ${firstNonEmpty(live.reporttime)}`,
      ],
    );
  }
  ...
  const sections = [
    formatBlock(
      ...
      [
        joinNonEmpty([firstNonEmpty(forecast.province), firstNonEmpty(forecast.city)], " / ")
        && `area: ${joinNonEmpty([firstNonEmpty(forecast.province), firstNonEmpty(forecast.city)], " / ")}`,
        firstNonEmpty(forecast.reporttime) && `report time: ${firstNonEmpty(forecast.reporttime)}`,
      ],
    ),
  ];
      ...
      formatBlockList(
        ...
        casts.map((cast, index) =>
          formatBlock(
            ...
            [
              joinNonEmpty([firstNonEmpty(cast.dayweather), firstNonEmpty(cast.nightweather)], " -> ")
              && `weather: ${joinNonEmpty([firstNonEmpty(cast.dayweather), firstNonEmpty(cast.nightweather)], " -> ")}`,
              joinNonEmpty([firstNonEmpty(cast.daytemp), firstNonEmpty(cast.nighttemp)], " / ")
              && `temperature: ${joinNonEmpty([firstNonEmpty(cast.daytemp), firstNonEmpty(cast.nighttemp)], " / ")} C`,
              joinNonEmpty([firstNonEmpty(cast.daywind), firstNonEmpty(cast.nightwind)], " / ")
...
function formatIpLocationResult(result: JsonObject) {
  ...
  return formatBlock("IP location result", [
    ...
    firstNonEmpty(payload.location) && `location: ${firstNonEmpty(payload.location)}`,
    firstNonEmpty(payload.rectangle) && `rectangle: ${firstNonEmpty(payload.rectangle)}`,
...
function formatCoordinateConvertResult(result: JsonObject) {
  return formatBlock("Coordinate conversion result", [
    firstNonEmpty(result.locations) && `locations: ${firstNonEmpty(result.locations)}`,
...
function formatRouteResult(title: string, result: JsonObject) {
  ...
  const sections = [
    formatBlock(title, [
      formatDistance(path.distance) && `distance: ${formatDistance(path.distance)}`,
      formatDurationSeconds(cost.duration ?? path.duration) && `duration: ${formatDurationSeconds(cost.duration ?? path.duration)}`,
      firstNonEmpty(path.strategy) && `strategy: ${firstNonEmpty(path.strategy)}`,
      firstNonEmpty(path.tolls, cost.tolls) && `tolls: ${firstNonEmpty(path.tolls, cost.tolls)}`,
      firstNonEmpty(path.toll_distance, cost.toll_distance) && `toll distance: ${firstNonEmpty(path.toll_distance, cost.toll_distance)}`,
      firstNonEmpty(route.taxi_cost, path.taxi_cost) && `taxi cost: ${firstNonEmpty(route.taxi_cost, path.taxi_cost)}`,
      firstNonEmpty(path.traffic_lights) && `traffic lights: ${firstNonEmpty(path.traffic_lights)}`,
      firstNonEmpty(path.restriction) && `restriction: ${firstNonEmpty(path.restriction)}`,
    ]),
  ];
...
function buildGeocodeQuery(input: JsonObject): QueryEntry[] {
  ...
  const city = readOptionalString(input.city) || readOptionalString(input.region);
...
function buildReverseGeocodeQuery(input: JsonObject): QueryEntry[] {
  return [
    ["location", normalizeCoordinatePair(readRequiredString(input, "location", "Location"), "Location")],
    ["radius", String(readInteger(input.radius, { defaultValue: 1000, min: 0, max: 3000 }) || 1000)],
...
function buildPoiTextSearchQuery(input: JsonObject): QueryEntry[] {
  ...
  const region = readOptionalString(input.region) || readOptionalString(input.city);
  const types = readJoinedStrings(input.types, "|") || readOptionalString(input.type);
  const showFields = readJoinedStrings(input.showFields, ",") || readOptionalString(input.show_fields);
  const cityLimit = readOptionalBoolean(input.cityLimit);
  const query: QueryEntry[] = [
    ["keywords", keywords],
    ["page_size", String(readInteger(input.pageSize || input.page_size, { defaultValue: 10, min: 1, max: MAX_POI_PAGE_SIZE }) || 10)],
    ["page_num", String(readInteger(input.pageNum || input.page_num, { defaultValue: 1, min: 1, max: 100 }) || 1)],
  ];
...
function buildPoiAroundQuery(input: JsonObject): QueryEntry[] {
  ...
  const query: QueryEntry[] = [
    ["location", normalizeCoordinatePair(readRequiredString(input, "location", "Location"), "Location")],
    ["radius", String(readInteger(input.radius, { defaultValue: 3000, min: 1, max: 50_000 }) || 3000)],
    ["sortrule", readOptionalString(input.sortRule) || readOptionalString(input.sortrule) || "distance"],
    ["page_size", String(readInteger(input.pageSize || input.page_size, { defaultValue: 10, min: 1, max: MAX_POI_PAGE_SIZE }) || 10)],
    ["page_num", String(readInteger(input.pageNum || input.page_num, { defaultValue: 1, min: 1, max: 100 }) || 1)],
  ];
...
function buildDistrictQuery(input: JsonObject): QueryEntry[] {
  ...
  return [
    ["keywords", keywords],
    ["subdistrict", String(readInteger(input.subdistrict, { defaultValue: 1, min: 0, max: 3 }) || 1)],
    ["page", String(readInteger(input.page, { defaultValue: 1, min: 1, max: 100 }) || 1)],
    ["offset", String(readInteger(input.offset, { defaultValue: 20, min: 1, max: 100 }) || 20)],
...
function buildCoordinateConvertQuery(input: JsonObject): QueryEntry[] {
  const coordsys = readOptionalString(input.coordSys) || readOptionalString(input.coordsys) || "autonavi";
...
function buildDrivingRouteQuery(input: JsonObject): QueryEntry[] {
  ...
  const waypoints = input.waypoints
    ...
    : undefined;
  const showFields = readJoinedStrings(input.showFields, ",") || readOptionalString(input.show_fields);
  const avoidRoad = readOptionalString(input.avoidRoad) || readOptionalString(input.avoidroad);
  const query: QueryEntry[] = [
    ["origin", normalizeCoordinatePair(readRequiredString(input, "origin", "Origin"), "Origin")],
    ["destination", normalizeCoordinatePair(readRequiredString(input, "destination", "Destination"), "Destination")],
  ];
...
const TOOL_SPECS: AmapToolSpec[] = [
  {
    name: "geocodeAddress",
    description: "Convert a free-form address into AMap coordinates and administrative area metadata.",
    path: "/v3/geocode/geo",
    params: [
      { name: "address", type: "string", description: "The address to geocode.", required: true },
      { name: "city", type: "string", description: "Optional city or region hint to disambiguate the address." },
    ],
  ...
  {
    name: "reverseGeocode",
    description: "Convert coordinates into a human-readable address, nearby POIs, and road context.",
    path: "/v3/geocode/regeo",
    params: [
      { name: "location", type: "string", description: "Longitude,latitude coordinates such as 116.397428,39.90923.", required: true },
      { name: "radius", type: "integer", description: "Search radius in meters. Default 1000.", minimum: 0, maximum: 3000 },
      { name: "extensions", type: "string", description: "Set to base or all. Default all.", enum: ["base", "all"] },
    ],
  ...
  },
  {
    ...
    description: "Search POIs by keyword, with optional region and type filters using AMap Place Search 2.0.",
    path: "/v5/place/text",
    params: [
      { name: "keywords", type: "string", description: "Search keywords such as Starbucks or Beijing University.", required: true },
      { name: "region", type: "string", description: "Optional region, city name, or adcode filter." },
      { name: "types", type: "string_array", description: "Optional AMap type codes to narrow the search." },
      { name: "cityLimit", type: "boolean", description: "When true, restrict results to the specified region." },
      { name: "pageSize", type: "integer", description: "Results per page. Default 10, max 25.", minimum: 1, maximum: MAX_POI_PAGE_SIZE },
      { name: "pageNum", type: "integer", description: "Page number starting from 1.", minimum: 1, maximum: 100 },
      { name: "showFields", type: "string_array", description: "Optional extra fields such as business, photos, or indoor." },
    ],
    buildQuery: buildPoiTextSearchQuery,
    formatResult: (result) => formatPoiSearchResult("POI text search results", result),
  },
  {
    name: "searchNearbyPlaces",
    description: "Search nearby POIs around a coordinate with optional keyword and type filters.",
    path: "/v5/place/around",
    params: [
      { name: "location", type: "string", description: "Center point in longitude,latitude format.", required: true },
      { name: "keywords", type: "string", description: "Optional keyword filter." },
      { name: "types", type: "string_array", description: "Optional AMap type codes to narrow the nearby search." },
      { name: "radius", type: "integer", description: "Radius in meters. Default 3000.", minimum: 1, maximum: 50000 },
      { name: "sortRule", type: "string", description: "Nearby sorting strategy. Usually distance or weight.", enum: ["distance", "weight"] },
      { name: "pageSize", type: "integer", description: "Results per page. Default 10, max 25.", minimum: 1, maximum: MAX_POI_PAGE_SIZE },
      { name: "pageNum", type: "integer", description: "Page number starting from 1.", minimum: 1, maximum: 100 },
      { name: "showFields", type: "string_array", description: "Optional extra fields such as business, photos, or indoor." },
    ],
    buildQuery: buildPoiAroundQuery,
    formatResult: (result) => formatPoiSearchResult("Nearby POI search results", result),
  },
  {
    name: "searchPlacesByPolygon",
    description: "Search POIs inside a polygon area, optionally filtered by keyword or type.",
    path: "/v5/place/polygon",
    params: [
      { name: "polygon", type: "string_array", description: "Polygon points as an array of longitude,latitude pairs. At least three points are required.", required: true },
      { name: "keywords", type: "string", description: "Optional keyword filter inside the polygon." },
      { name: "types", type: "string_array", description: "Optional AMap type codes to narrow the polygon search." },
      { name: "pageSize", type: "integer", description: "Results per page. Default 10, max 25.", minimum: 1, maximum: MAX_POI_PAGE_SIZE },
      { name: "pageNum", type: "integer", description: "Page number starting from 1.", minimum: 1, maximum: 100 },
      { name: "showFields", type: "string_array", description: "Optional extra fields such as business, photos, or indoor." },
    ],
    buildQuery: buildPoiPolygonQuery,
    formatResult: (result) => formatPoiSearchResult("Polygon POI search results", result),
  },
  {
    ...
    params: [
      { name: "ids", type: "string_array", description: "One or more AMap place IDs.", required: true },
      { name: "showFields", type: "string_array", description: "Optional extra fields such as business, photos, or indoor." },
    ],
    buildQuery: buildPlaceDetailQuery,
    formatResult: (result) => formatPoiSearchResult("POI detail results", result),
  },
  {
    name: "getPlaceSuggestions",
    description: "Get autocomplete suggestions for user-entered place keywords.",
    path: "/v3/assistant/inputtips",
    params: [
      { name: "keywords", type: "string", description: "Partial place keyword to autocomplete.", required: true },
      { name: "city", type: "string", description: "Optional city or city code hint." },
      { name: "cityLimit", type: "boolean", description: "When true, restrict suggestions to the specified city." },
      { name: "datatype", type: "string", description: "Optional suggestion type filter, such as all, poi, bus, or busline." },
    ],
  ...
  },
  {
    ...
    description: "Look up administrative districts, optionally including nested subdistricts.",
    path: "/v3/config/district",
    params: [
      { name: "keywords", type: "string", description: "District keyword, city name, or adcode.", required: true },
      { name: "subdistrict", type: "integer", description: "Nested district depth from 0 to 3. Default 1.", minimum: 0, maximum: 3 },
      { name: "page", type: "integer", description: "Page number starting from 1.", minimum: 1, maximum: 100 },
      { name: "offset", type: "integer", description: "Results per page. Default 20.", minimum: 1, maximum: 100 },
      { name: "extensions", type: "string", description: "Set to base or all. Default base.", enum: ["base", "all"] },
    ],
  ...
  {
    name: "getWeather",
    description: "Fetch live weather or a multi-day forecast for a city code or adcode.",
    path: "/v3/weather/weatherInfo",
    params: [
      { name: "city", type: "string", description: "City code or adcode, such as 110101.", required: true },
      { name: "extensions", type: "string", description: "Set to base for live weather or all for forecasts.", enum: ["base", "all"] },
    ],
  ...
  {
    name: "locateIp",
    description: "Resolve an IP address to an approximate geographic area using AMap IP location.",
    path: "/v5/ip/location",
    params: [
      { name: "ip", type: "string", description: "Optional IPv4 address. When omitted, AMap uses the caller IP." },
      { name: "type", type: "integer", description: "Optional location type. Commonly 4.", minimum: 1, maximum: 4 },
    ],
  ...
  },
  {
    ...
    description: "Convert coordinates from GPS, Baidu, Mapbar, or AMap formats into AMap coordinates.",
    path: "/v3/assistant/coordinate/convert",
    params: [
      { name: "locations", type: "string_array", description: "One or more longitude,latitude coordinates to convert.", required: true },
      { name: "coordSys", type: "string", description: "Source coordinate system. Default autonavi.", enum: ["gps", "mapbar", "baidu", "autonavi"] },
    ],
  ...
  },
  {
    ...
    description: "Plan a driving route between two coordinates with optional strategy, waypoints, and avoidance settings.",
    path: "/v5/direction/driving",
    params: [
      { name: "origin", type: "string", description: "Origin in longitude,latitude format.", required: true },
      { name: "destination", type: "string", description: "Destination in longitude,latitude format.", required: true },
      { name: "strategy", type: "string", description: "Optional AMap driving strategy code." },
      { name: "waypoints", type: "string_array", description: "Optional waypoint coordinates in travel order." },
      { name: "avoidRoad", type: "string", description: "Optional road name to avoid." },
      { name: "showFields", type: "string_array", description: "Optional extra fields such as cost, polyline, tmcs, or navi." },
    ],
  ...
  },
  {
    ...
    description: "Plan a walking route between two coordinates, optionally enabling indoor routing.",
    path: "/v5/direction/walking",
    params: [
      { name: "origin", type: "string", description: "Origin in longitude,latitude format.", required: true },
      { name: "destination", type: "string", description: "Destination in longitude,latitude format.", required: true },
      { name: "isIndoor", type: "boolean", description: "Set true to prefer indoor walking segments when supported." },
      { name: "showFields", type: "string_array", description: "Optional extra fields such as cost, polyline, or navi." },
    ],
