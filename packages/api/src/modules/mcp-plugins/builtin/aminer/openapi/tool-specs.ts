...
class AminerApiError extends Error {
  ...
  constructor(message: string, options?: { code?: number; isAuthError?: boolean }) {
    super(message);
...
function summarizeArray(
  value: unknown,
  options?: { limit?: number; extractor?: (item: unknown) => string | undefined },
) {
...
function finalizeOutput(
  ...
  const blocks = [title, ...sections.filter((section): section is string => Boolean(section))];
...
function renderScholarItem(item: unknown, index: number) {
  ...
  return [
    ...
    firstNonEmpty(scholar.org_zh, scholar.org) ? `Organization: ${firstNonEmpty(scholar.org_zh, scholar.org)}` : null,
    readOptionalNumber(scholar.n_citation) !== undefined
      ? `Citations: ${readOptionalNumber(scholar.n_citation)}`
      : null,
    summarizeArray(scholar.interests) ? `Interests: ${summarizeArray(scholar.interests, { limit: 6 })}` : null,
  ]
    .filter(Boolean)
    .join("\n");
}
...
function renderPaperItem(item: unknown, index: number) {
  ...
  return [
    ...
    firstNonEmpty(paper.id, paper._id) ? `ID: ${firstNonEmpty(paper.id, paper._id)}` : null,
    firstNonEmpty(paper.doi) ? `DOI: ${firstNonEmpty(paper.doi)}` : null,
    readOptionalNumber(paper.year) !== undefined ? `Year: ${readOptionalNumber(paper.year)}` : null,
    venue ? `Venue: ${venue}` : null,
    authors ? `Authors: ${authors}` : null,
    readOptionalNumber(paper.n_citation) !== undefined
      ? `Citations: ${readOptionalNumber(paper.n_citation)}`
      : null,
    summarizeArray(paper.keywords, { limit: 6 }) ? `Keywords: ${summarizeArray(paper.keywords, { limit: 6 })}` : null,
...
function renderPatentItem(item: unknown, index: number) {
  ...
  return [
    ...
    firstNonEmpty(patent.patent_no, patent.number, patent.application_no)
      ...
      : null,
    firstNonEmpty(patent.country) ? `Country: ${firstNonEmpty(patent.country)}` : null,
    cleanTextValue(patent.abstract) ? `Abstract: ${cleanTextValue(patent.abstract)}` : null,
  ]
    .filter(Boolean)
    .join("\n");
}
...
function renderOrganizationItem(item: unknown, index: number) {
  ...
  return [
    ...
    firstNonEmpty(organization.org_id, organization.id) ? `ID: ${firstNonEmpty(organization.org_id, organization.id)}` : null,
    summarizeArray(organization.aliases, { limit: 5 }) ? `Aliases: ${summarizeArray(organization.aliases, { limit: 5 })}` : null,
    summarizeArray(organization.acronyms, { limit: 5 }) ? `Acronyms: ${summarizeArray(organization.acronyms, { limit: 5 })}` : null,
    firstNonEmpty(organization.type) ? `Type: ${firstNonEmpty(organization.type)}` : null,
    cleanTextValue(organization.location) ? `Location: ${cleanTextValue(organization.location)}` : null,
  ]
    .filter(Boolean)
    .join("\n");
}
...
function renderVenueItem(item: unknown, index: number) {
  ...
  return [
    ...
    summarizeArray(venue.alias, { limit: 5 }) ? `Aliases: ${summarizeArray(venue.alias, { limit: 5 })}` : null,
  ]
    .filter(Boolean)
    .join("\n");
}
...
function renderProjectItem(item: unknown, index: number) {
  ...
  const titles = Array.isArray(project.titles)
    ? project.titles
        .map((title) => {
          ...
          return firstNonEmpty(titleObject.title_zh, titleObject.title, titleObject.name);
        })
  ...
  return [
    ...
    titles.length > 1 ? `Additional titles: ${titles.slice(1, 4).join(", ")}` : null,
    firstNonEmpty(project.project_source) ? `Source: ${firstNonEmpty(project.project_source)}` : null,
    firstNonEmpty(project.country) ? `Country: ${firstNonEmpty(project.country)}` : null,
...
function formatListResult(
  ...
  const sections: Array<string | null | undefined> = [
    typeof total === "number" ? `Total results: ${total}` : `Returned results: ${items.length}`,
...
function formatScholarDetail(result: JsonObject) {
  ...
  const sections = [
    formatSection("Scholar", [
      ...
      firstNonEmpty(scholar.id) ? `ID: ${firstNonEmpty(scholar.id)}` : null,
      firstNonEmpty(scholar.position_zh, scholar.position) ? `Position: ${firstNonEmpty(scholar.position_zh, scholar.position)}` : null,
      summarizeArray(scholar.org_zhs, { limit: 4 }) || summarizeArray(scholar.orgs, { limit: 4 })
...
function formatScholarProfile(result: JsonObject) {
  ...
  const sections = [
    formatSection("Scholar profile", [
      firstNonEmpty(profile.id) ? `Scholar ID: ${firstNonEmpty(profile.id)}` : null,
      summarizeArray(profile.interests, { limit: 12 }) ? `Interests: ${summarizeArray(profile.interests, { limit: 12 })}` : null,
      summarizeArray(profile.domain, { limit: 8 }) ? `Domains: ${summarizeArray(profile.domain, { limit: 8 })}` : null,
      summarizeArray(profile.research_fields, { limit: 8 })
        ? `Research fields: ${summarizeArray(profile.research_fields, { limit: 8 })}`
        : null,
    ]),
    Object.keys(profile).length > 0 ? `Raw profile excerpt:\n${stringifyClipped(profile)}` : null,
  ];
...
function formatPaperDetail(result: JsonObject) {
  ...
  const sections = [
    ...
    summarizeArray(paper.keywords, { limit: 12 }) ? `Keywords: ${summarizeArray(paper.keywords, { limit: 12 })}` : null,
...
function formatPatentDetail(result: JsonObject) {
  ...
  const sections = [
    ...
    cleanTextValue(patent.abstract_zh || patent.abstract)
      ? `Abstract:\n${cleanTextValue(patent.abstract_zh || patent.abstract)}`
      : null,
    Object.keys(patent).length > 0 ? `Raw detail excerpt:\n${stringifyClipped(patent)}` : null,
  ];
  return finalizeOutput("Patent detail", sections, result);
}

function formatOrganizationDetails(result: JsonObject) {
  return formatListResult("Organization details", result, renderOrganizationItem);
}
...
function formatVenueDetail(result: JsonObject) {
  ...
  const sections = [
    formatSection("Venue", [
      ...
      firstNonEmpty(venue.issn) ? `ISSN: ${firstNonEmpty(venue.issn)}` : null,
      firstNonEmpty(venue.type) ? `Type: ${firstNonEmpty(venue.type)}` : null,
      summarizeArray(venue.alias, { limit: 8 }) ? `Aliases: ${summarizeArray(venue.alias, { limit: 8 })}` : null,
    ]),
    Object.keys(venue).length > 0 ? `Raw detail excerpt:\n${stringifyClipped(venue)}` : null,
  ];
...
function formatDeepResearchResult(result: JsonObject | string) {
  ...
  if (events.length === 0) {
    ...
    return finalizeOutput("Deep academic research", [`Raw response:\n${stringifyClipped(result)}`], result);
  }
  ...
  const sections = [
    ...
    thoughtChunks.length > 0 ? `Thinking trace:\n${thoughtChunks.join("\n")}` : null,
...
function getConfiguredToken(config: JsonObject) {
  ...
  if (apiKey && userId) {
    const ttlSeconds = readInteger(config.tokenTtlSeconds, {
      defaultValue: 7200,
      min: 60,
      max: 86_400,
    }) || 7200;
  ...
  const token = readOptionalString(config.apiToken)
    || readOptionalString(config.accessToken)
    || readOptionalString(config.token);
  if (!token) {
    throw new Error("AMiner credentials not configured. Set apiKey + userId, or provide apiToken as a fallback.");
  }
  return token;
}
...
async function fetchAminer(
  ...
  const timeoutMs = readInteger(config.timeoutMs, {
    ...
    try {
      const response = await fetch(url, {
        ...
        headers: {
          ...
          ...(spec.method === "POST" ? { "Content-Type": "application/json;charset=utf-8" } : {}),
        },
        body: spec.method === "POST" ? JSON.stringify(request.body || {}) : undefined,
      ...
      if (spec.stream || (response.headers.get("content-type") || "").includes("text/event-stream")) {
        if (parsed && parsed.success === false) {
          throw new AminerApiError(
            `AMiner ${spec.name} failed (${readOptionalNumber(parsed.code) || "unknown"}): ${firstNonEmpty(parsed.msg, parsed.message) || "Unknown error"}`,
            {
              ...
              isAuthError: isAuthErrorCode(readInteger(parsed.code), firstNonEmpty(parsed.msg, parsed.message)),
            },
          );
        }
        return text;
      ...
      if (parsed.success === false || (parsed.code !== undefined && Number(parsed.code) !== 200)) {
        const code = readInteger(parsed.code);
        const message = firstNonEmpty(parsed.msg, parsed.message) || "Unknown error";
        throw new AminerApiError(
...
function buildScholarSearchRequest(input: JsonObject) {
  ...
  const body: JsonObject = {
    ...
    size: readInteger(input.limit || input.size, {
      defaultValue: 10,
      min: 1,
      max: 10,
    }) || 10,
...
function buildSingleIdQuery(input: JsonObject, sourceKey: string, targetKey = "id", label = "ID") {
  const id = readRequiredString(input, sourceKey, label);
  return { query: [[targetKey, id]] as QueryEntry[] };
}

function buildSingleIdBody(input: JsonObject, sourceKey: string, targetKey = "id", label = "ID") {
  const id = readRequiredString(input, sourceKey, label);
...
function buildPaperSearchByTitleRequest(input: JsonObject) {
  ...
  return {
    query: [
      ...
      ["size", String(readInteger(input.size || input.limit, {
        defaultValue: 10,
        min: 1,
        max: 100,
      }) || 10)],
...
function buildPaperQaSearchRequest(input: JsonObject) {
  const useTopicSearch = readOptionalBoolean(input.useTopicSearch ?? input.use_topic) ?? false;
  ...
  requireAtLeastOne(
    [query, titles, doi, requiredTopics, boostTopics, softBoostTopics, authorTerms, organizationTerms, authorIds, organizationIds, venueIds],
    "Provide query or at least one structured search field.",
  );

  const body: JsonObject = {
    use_topic: useTopicSearch,
    size: readInteger(input.size || input.limit, {
      defaultValue: 20,
      min: 1,
      max: 100,
    }) || 20,
    offset: readInteger(input.offset, { defaultValue: 0, min: 0, max: 10_000 }) || 0,
  };
  if (typeof requiredTopics === "string") body.topic_high = requiredTopics;
  else if (requiredTopics.length > 0) body.topic_high = JSON.stringify(requiredTopics);
  if (typeof boostTopics === "string") body.topic_middle = boostTopics;
  else if (boostTopics.length > 0) body.topic_middle = JSON.stringify(boostTopics);
  if (typeof softBoostTopics === "string") body.topic_low = softBoostTopics;
  else if (softBoostTopics.length > 0) body.topic_low = JSON.stringify(softBoostTopics);
  ...
  const preferHighlyCited = readOptionalBoolean(input.preferHighlyCited ?? input.n_citation_flag);
  if (preferHighlyCited !== undefined) body.n_citation_flag = preferHighlyCited;
  const sortByCitations = readOptionalBoolean(input.sortByCitations ?? input.force_citation_sort);
  if (sortByCitations !== undefined) body.force_citation_sort = sortByCitations;
  const sortByYear = readOptionalBoolean(input.sortByYear ?? input.force_year_sort);
...
function buildDeepResearchRequest(input: JsonObject) {
  ...
  const corpusType = readInteger(input.corpusType || input.type, { defaultValue: 1 });
  if (!corpusType || !DEEP_RESEARCH_CORPUS_TYPES.includes(corpusType)) {
    throw new Error("corpusType must be one of 1 (AMiner full corpus), 2 (preprint), or 3 (medical).");
  }
  const body: JsonObject = {
    message,
    type: corpusType,
...
function buildOrganizationNameRequest(input: JsonObject) {
  const organization = readRequiredString(input, "organization", "Organization text");
  return { body: { org: organization } };
}

function buildOrganizationPeopleRequest(input: JsonObject) {
  const organizationId = readRequiredString(input, "organizationId", "Organization ID");
  return {
    query: [
      ["org_id", organizationId],
      ["offset", String(readInteger(input.offset, { defaultValue: 0, min: 0 }) || 0)],
    ] as QueryEntry[],
  };
}
...
function buildOrganizationPatentRequest(input: JsonObject) {
  ...
  return {
    query: [
      ["id", organizationId],
      ["page", String(readInteger(input.page, { defaultValue: 1, min: 1 }) || 1)],
      ["page_size", String(readInteger(input.pageSize, {
        defaultValue: 20,
        min: 1,
        max: 100,
      }) || 20)],
...
function buildVenuePapersRequest(input: JsonObject) {
  ...
  const body: JsonObject = {
    id: venueId,
    offset: readInteger(input.offset, { defaultValue: 0, min: 0 }) || 0,
    limit: readInteger(input.limit || input.size, {
      defaultValue: 20,
      min: 1,
      max: 100,
    }) || 20,
...
const TOOL_SPECS: AminerToolSpec[] = [
  {
    name: "searchScholars",
    description: "Search AMiner scholars by name, organization name, or organization IDs. Returns scholar IDs, names, organizations, interests, and citation counts.",
    method: "POST",
    path: "/person/search",
    params: [
      { name: "name", type: "string", description: "Scholar name to search for." },
      { name: "organization", type: "string", description: "Optional organization name filter." },
      { name: "organizationIds", type: "string_array", description: "Optional AMiner organization IDs to filter by." },
      { name: "offset", type: "integer", description: "Result offset. Defaults to 0.", minimum: 0 },
      { name: "limit", type: "integer", description: "Number of results to return. Defaults to 10, max 10.", minimum: 1, maximum: 10 },
    ],
    buildRequest: buildScholarSearchRequest,
    formatResult: (result) => formatListResult("Scholar search results", asObject(result), renderScholarItem),
  },
  {
    name: "getScholarDetail",
    description: "Get detailed scholar information by AMiner scholar ID, including biography, education, position, honors, and organizations.",
    method: "GET",
    path: "/person/detail",
    params: [{ name: "scholarId", type: "string", description: "AMiner scholar ID.", required: true }],
    buildRequest: (input) => buildSingleIdQuery(input, "scholarId", "id", "Scholar ID"),
    formatResult: (result) => formatScholarDetail(asObject(result)),
  },
  {
    name: "getScholarProfile",
    description: "Get AMiner scholar profile signals such as interests, domains, and structured background by scholar ID.",
    method: "GET",
    path: "/person/figure",
    params: [{ name: "scholarId", type: "string", description: "AMiner scholar ID.", required: true }],
    buildRequest: (input) => buildSingleIdQuery(input, "scholarId", "id", "Scholar ID"),
  ...
  },
  {
    ...
    params: [{ name: "scholarId", type: "string", description: "AMiner scholar ID.", required: true }],
    buildRequest: (input) => buildSingleIdQuery(input, "scholarId", "id", "Scholar ID"),
    formatResult: (result) => formatListResult("Scholar papers", asObject(result), renderPaperItem),
  },
  {
    ...
    params: [{ name: "scholarId", type: "string", description: "AMiner scholar ID.", required: true }],
    buildRequest: (input) => buildSingleIdQuery(input, "scholarId", "id", "Scholar ID"),
    formatResult: (result) => formatListResult("Scholar patents", asObject(result), renderPatentItem),
  },
  {
    ...
    params: [{ name: "scholarId", type: "string", description: "AMiner scholar ID.", required: true }],
    buildRequest: (input) => buildSingleIdQuery(input, "scholarId", "id", "Scholar ID"),
    formatResult: (result) => formatListResult("Scholar projects", asObject(result), renderProjectItem),
  },
  {
    name: "searchPapersByTitle",
    description: "Search papers by title with lightweight paper metadata output.",
    method: "GET",
    path: "/paper/search",
    params: [
      { name: "title", type: "string", description: "Paper title or title fragment.", required: true },
      { name: "page", type: "integer", description: "Page number. Defaults to 1.", minimum: 1 },
      { name: "size", type: "integer", description: "Page size. Defaults to 10.", minimum: 1, maximum: 100 },
    ],
    buildRequest: buildPaperSearchByTitleRequest,
    formatResult: (result) => formatListResult("Paper title search results", asObject(result), renderPaperItem),
  },
  {
    name: "searchPapersAdvanced",
    description: "Advanced paper search with title, keyword, abstract, author, organization, venue, and sort options.",
    method: "GET",
    path: "/paper/search/pro",
    params: [
      { name: "title", type: "string", description: "Optional paper title filter." },
      { name: "keyword", type: "string", description: "Optional keyword filter." },
      { name: "abstractText", type: "string", description: "Optional abstract text filter." },
      { name: "author", type: "string", description: "Optional author name filter." },
      { name: "organization", type: "string", description: "Optional organization filter." },
      { name: "venue", type: "string", description: "Optional venue filter." },
      { name: "sortBy", type: "string", description: "Optional sort order.", enum: PAPER_SORT_OPTIONS },
      { name: "page", type: "integer", description: "Page number. Defaults to 1.", minimum: 1 },
      { name: "size", type: "integer", description: "Page size. Defaults to 10.", minimum: 1, maximum: 100 },
    ],
    buildRequest: buildPaperSearchAdvancedRequest,
    formatResult: (result) => formatListResult("Advanced paper search results", asObject(result), renderPaperItem),
  },
  {
    name: "searchPapers",
    description: "Search papers by keyword, author, or venue with richer paper metadata than the title-only endpoint.",
    method: "GET",
    ...
    formatResult: (result) => formatListResult("Paper search results", asObject(result), renderPaperItem),
  },
  {
    name: "searchPapersByKeywords",
    description: "Search papers by multiple keywords. Good for broad recall across related concepts.",
    method: "GET",
    path: "/paper/list/citation/by/keywords",
    params: [
      { name: "keywords", type: "string_array", description: "One or more keywords.", required: true },
      { name: "page", type: "integer", description: "Page number. Defaults to 1.", minimum: 1 },
      { name: "size", type: "integer", description: "Page size. Defaults to 10.", minimum: 1, maximum: 100 },
    ],
    buildRequest: buildPaperKeywordBatchRequest,
    formatResult: (result) => formatListResult("Paper keyword search results", asObject(result), renderPaperItem),
  },
  {
    name: "findPapersByVenueYear",
    description: "Find papers for a specific venue and year, using the AMiner endpoint that returns detailed paper records by venue ID and year.",
    method: "GET",
    path: "/paper/platform/allpubs/more/detail/by/ts/org/venue",
    params: [
      { name: "year", type: "integer", description: "Publication year.", required: true, minimum: 0 },
      { name: "venueId", type: "string", description: "AMiner venue ID.", required: true },
    ],
    buildRequest: buildPapersByVenueYearRequest,
    formatResult: (result) => formatListResult("Papers by venue and year", asObject(result), renderPaperItem),
  },
  {
    name: "getPaperInfoBatch",
    description: "Get lightweight paper info for up to 100 paper IDs in one request.",
    method: "POST",
    path: "/paper/info",
    params: [
      { name: "paperIds", type: "string_array", description: "List of AMiner paper IDs, up to 100 items.", required: true },
    ],
    buildRequest: buildPaperInfoBatchRequest,
    formatResult: (result) => formatListResult("Paper info batch", asObject(result), renderPaperItem),
  },
  {
    name: "getPaperDetail",
    description: "Get detailed paper metadata by AMiner paper ID, including abstract, authors, DOI, keywords, and venue data.",
    method: "GET",
    path: "/paper/detail",
    params: [{ name: "paperId", type: "string", description: "AMiner paper ID.", required: true }],
    buildRequest: (input) => buildSingleIdQuery(input, "paperId", "id", "Paper ID"),
  ...
  },
  {
    ...
    params: [{ name: "paperId", type: "string", description: "AMiner paper ID.", required: true }],
    buildRequest: (input) => buildSingleIdQuery(input, "paperId", "id", "Paper ID"),
    formatResult: (result) => formatListResult("Paper references", asObject(result), renderPaperItem),
  },
  {
    name: "askAcademicPapers",
    description: "Search academic papers using AMiner's academic QA search endpoint. Supports natural-language queries plus structured filters and nested topic groups.",
    method: "POST",
    path: "/paper/qa/search",
    params: [
      { name: "useTopicSearch", type: "boolean", description: "Whether to use the structured topic search mode. Defaults to false." },
      { name: "requiredTopics", type: "string_matrix", description: "Nested topic groups that must appear. Inner arrays are OR, outer arrays are AND." },
      { name: "boostTopics", type: "string_matrix", description: "Nested topic groups that strongly boost results." },
      { name: "softBoostTopics", type: "string_matrix", description: "Nested topic groups that softly boost results." },
      { name: "query", type: "string", description: "Natural-language research question." },
      { name: "titles", type: "string_array", description: "Optional title filters." },
      { name: "doi", type: "string", description: "Optional DOI filter." },
      { name: "years", type: "number_array", description: "Optional publication year filters." },
      { name: "sciOnly", type: "boolean", description: "Restrict to SCI papers." },
      { name: "preferHighlyCited", type: "boolean", description: "Boost highly cited papers." },
      { name: "sortByCitations", type: "boolean", description: "Force citation sort." },
      { name: "sortByYear", type: "boolean", description: "Force year sort." },
      { name: "authorTerms", type: "string_array", description: "Author name variants to match." },
      { name: "organizationTerms", type: "string_array", description: "Organization name variants to match." },
      { name: "authorIds", type: "string_array", description: "AMiner author IDs to match." },
      { name: "organizationIds", type: "string_array", description: "AMiner organization IDs to match." },
      { name: "venueIds", type: "string_array", description: "AMiner venue IDs to match." },
      { name: "size", type: "integer", description: "Result count. Defaults to 20, max 100.", minimum: 1, maximum: 100 },
      { name: "offset", type: "integer", description: "Offset. Defaults to 0.", minimum: 0, maximum: 10000 },
    ],
    buildRequest: buildPaperQaSearchRequest,
    formatResult: (result) => formatListResult("Academic QA paper search results", asObject(result), renderPaperItem),
  },
  {
    name: "runDeepAcademicResearch",
    description: "Run AMiner deep academic research over the selected corpus. Returns streamed thinking and answer chunks collapsed into a final readable transcript.",
    method: "POST",
    path: "/paper/deep_research",
    stream: true,
    params: [
      { name: "message", type: "string", description: "Research prompt or question.", required: true },
      { name: "corpusType", type: "integer", description: "1 = AMiner full corpus, 2 = preprint, 3 = medical.", required: true, enum: DEEP_RESEARCH_CORPUS_TYPES },
      { name: "webSearch", type: "boolean", description: "Whether to allow web resources." },
    ],
  ...
  },
  {
    ...
    path: "/patent/search",
    params: [
      { name: "query", type: "string", description: "Patent search query.", required: true },
      { name: "page", type: "integer", description: "Page number. Defaults to 1.", minimum: 1 },
      { name: "size", type: "integer", description: "Page size. Defaults to 10.", minimum: 1, maximum: 100 },
    ],
    buildRequest: buildPatentSearchRequest,
    formatResult: (result) => formatListResult("Patent search results", asObject(result), renderPatentItem),
  },
  {
    name: "getPatentInfo",
    ...
    method: "GET",
    path: "/patent/info",
    params: [{ name: "patentId", type: "string", description: "AMiner patent ID.", required: true }],
    buildRequest: (input) => buildSingleIdQuery(input, "patentId", "id", "Patent ID"),
    formatResult: (result) => formatListResult("Patent info", asObject(result), renderPatentItem),
  },
  {
  ...
  },
  {
    ...
    description: "Search AMiner organizations by one or more organization strings.",
    method: "POST",
    path: "/organization/search",
    params: [
      { name: "query", type: "string", description: "Single organization query string." },
      { name: "queries", type: "string_array", description: "Multiple organization query strings." },
    ],
    buildRequest: buildOrganizationSearchRequest,
    formatResult: (result) => formatListResult("Organization search results", asObject(result), renderOrganizationItem),
  },
  {
    name: "getOrganizationDetails",
    description: "Get detailed AMiner organization records by one or more organization IDs.",
    method: "POST",
    path: "/organization/detail",
    params: [
      { name: "organizationId", type: "string", description: "Single organization ID." },
      { name: "organizationIds", type: "string_array", description: "Multiple organization IDs." },
    ],
  ...
  },
  {
    ...
    description: "Normalize a raw organization string into an AMiner standard organization name.",
    method: "POST",
    path: "/organization/na",
    params: [{ name: "organization", type: "string", description: "Organization text to normalize.", required: true }],
    buildRequest: buildOrganizationNameRequest,
    formatResult: (result) => finalizeOutput("Organization normalization", [`Raw response:\n${stringifyClipped(asObject(result).data)}`], asObject(result)),
  },
  {
    name: "normalizeOrganizationHierarchy",
    description: "Extract first-level and second-level organization IDs from a raw organization string.",
    method: "POST",
    path: "/organization/na/pro",
    params: [{ name: "organization", type: "string", description: "Organization text to normalize.", required: true }],
    buildRequest: buildOrganizationNameRequest,
    formatResult: (result) => finalizeOutput("Organization hierarchy normalization", [`Raw response:\n${stringifyClipped(asObject(result).data)}`], asObject(result)),
  },
  {
    ...
    params: [
      { name: "organizationId", type: "string", description: "AMiner organization ID.", required: true },
      { name: "offset", type: "integer", description: "Result offset. Defaults to 0.", minimum: 0 },
    ],
    buildRequest: buildOrganizationPeopleRequest,
    formatResult: (result) => formatListResult("Organization scholars", asObject(result), renderScholarItem),
  },
  {
    ...
    buildRequest: buildOrganizationPeopleRequest,
    formatResult: (result) => formatListResult("Organization papers", asObject(result), renderPaperItem),
  },
  {
    ...
    params: [
      { name: "organizationId", type: "string", description: "AMiner organization ID.", required: true },
      { name: "page", type: "integer", description: "Page number. Defaults to 1.", minimum: 1 },
      { name: "pageSize", type: "integer", description: "Page size. Defaults to 20.", minimum: 1, maximum: 100 },
    ],
    buildRequest: buildOrganizationPatentRequest,
    formatResult: (result) => formatListResult("Organization patents", asObject(result), renderPatentItem),
  },
  {
    name: "searchVenues",
    ...
    method: "POST",
    path: "/venue/search",
    params: [{ name: "name", type: "string", description: "Venue name.", required: true }],
    buildRequest: buildVenueSearchRequest,
    formatResult: (result) => formatListResult("Venue search results", asObject(result), renderVenueItem),
  },
  {
    name: "getVenueDetail",
    ...
    method: "POST",
    path: "/venue/detail",
    params: [{ name: "venueId", type: "string", description: "AMiner venue ID.", required: true }],
    buildRequest: (input) => buildSingleIdBody(input, "venueId", "id", "Venue ID"),
    formatResult: (result) => formatVenueDetail(asObject(result)),
  },
  {
    name: "listVenuePapers",
    description: "List papers associated with a venue, optionally filtered by year.",
    method: "POST",
    path: "/venue/paper/relation",
    params: [
      { name: "venueId", type: "string", description: "AMiner venue ID.", required: true },
      { name: "offset", type: "integer", description: "Offset. Defaults to 0.", minimum: 0 },
      { name: "limit", type: "integer", description: "Number of results. Defaults to 20.", minimum: 1, maximum: 100 },
      { name: "year", type: "integer", description: "Optional publication year filter.", minimum: 0 },
    ],
    buildRequest: buildVenuePapersRequest,
    formatResult: (result) => formatListResult("Venue papers", asObject(result), renderPaperItem),
  },
];
