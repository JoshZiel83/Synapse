...
const SEARCH_RECENCY_FILTERS = ['oneDay', 'oneWeek', 'oneMonth', 'oneYear', 'noLimit'];
...
const TOOL_DEFINITIONS: ToolDefinition[] = [
  {
    ...
    parameters: {
      ...
      properties: {
        query: {
          type: 'string',
          description: 'Search query. The official API recommends keeping it within 70 characters.',
        },
        searchEngine: {
          type: 'string',
          description: 'Search engine backend. If omitted, this wrapper defaults to search_std.',
          enum: SEARCH_ENGINES,
        },
        searchIntent: {
          type: 'boolean',
          description: 'Whether to let the API detect search intent before executing the search. If omitted, this wrapper defaults to false.',
        },
        count: {
          type: 'integer',
          description: 'Number of results to return. Official range: 1-50. If omitted, this wrapper defaults to 10.',
        },
        domainFilter: {
          type: 'string',
          description: 'Optional domain whitelist filter, e.g. www.example.com.',
        },
        recencyFilter: {
          type: 'string',
          description: 'Optional publish-time filter for results. If omitted, this wrapper defaults to noLimit.',
          enum: SEARCH_RECENCY_FILTERS,
        },
        contentSize: {
          type: 'string',
          description: 'How much content to include per result. medium returns shorter summaries; high returns richer result context.',
          enum: CONTENT_SIZES,
        },
        requestId: {
          type: 'string',
...
export const searchFeature: SubFeature = {
  ...
  async execute(toolName: string, input: Record<string, unknown>, config: Record<string, unknown>): Promise<string> {
    ...
    const searchEngine = SEARCH_ENGINES.includes(input.searchEngine as string)
      ...
      : 'search_std';
    const recencyFilter = SEARCH_RECENCY_FILTERS.includes(input.recencyFilter as string)
    ...
    try {
      ...
      const result = await response.json() as {
        request_id?: string;
        search_intent?: Array<{ query?: string; intent?: string; keywords?: string }>;
        search_result?: Array<{
          title?: string;
          content?: string;
          link?: string;
      ...
      if (result.search_intent?.length) {
        sections.push(
          [
            'Search intent:',
            ...result.search_intent.map((item, index) => (
