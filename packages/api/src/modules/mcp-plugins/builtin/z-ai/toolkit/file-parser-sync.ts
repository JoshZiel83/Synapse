...
const FILE_TYPES = [
  'WPS', 'PDF', 'DOCX', 'DOC', 'XLS', 'XLSX', 'PPT', 'PPTX',
  'PNG', 'JPG', 'JPEG', 'CSV', 'TXT', 'MD', 'HTML', 'BMP',
  'GIF', 'WEBP', 'HEIC', 'EPS', 'ICNS', 'IM', 'PCX', 'PPM',
  'TIFF', 'XBM', 'HEIF', 'JP2',
];
...
const TOOL_DEFINITIONS: ToolDefinition[] = [
  {
    ...
    parameters: {
      type: 'object',
      properties: {
        fileRef: fileRefProperty('Document or image FileRef to parse with the synchronous parser.'),
        fileType: {
          type: 'string',
          description: 'Optional explicit file type. If omitted, the server infers it from the FileRef filename/mime type.',
          enum: FILE_TYPES,
        },
      },
      required: ['fileRef'],
...
export const fileParserSyncFeature: SubFeature = {
  ...
  async execute(_toolName: string, input: Record<string, unknown>, config: Record<string, unknown>): Promise<string | unknown[]> {
