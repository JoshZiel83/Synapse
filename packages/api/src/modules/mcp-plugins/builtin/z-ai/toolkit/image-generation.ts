...
const VALID_MODELS = ['glm-image', 'cogview-4-250304', 'cogview-4', 'cogview-3-flash'];
...
const TOOL_DEFINITIONS: ToolDefinition[] = [
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        quality: {
          type: 'string',
          description: 'Image quality. "hd": finer detail (~20s). "standard": faster (~5-10s). glm-image only supports hd.',
          enum: ['standard', 'hd'],
        },
        size: {
          type: 'string',
...
export const imageGenFeature: SubFeature = {
  ...
  async execute(toolName: string, input: Record<string, unknown>, config: Record<string, unknown>): Promise<string | unknown[]> {
    ...
    const model = VALID_MODELS.includes(input.model as string) ? (input.model as string) : DEFAULT_MODEL;
    const prompt = input.prompt as string;

    // Determine defaults based on model
    const isGlmImage = model === 'glm-image';
    const size = (input.size as string) || (isGlmImage ? '1280x1280' : '1024x1024');
