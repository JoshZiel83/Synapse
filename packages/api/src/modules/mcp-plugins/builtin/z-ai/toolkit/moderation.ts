...
import { fileRefProperty, fileRefsProperty, resolveFileRefRecord } from '../../../file-ref.js';
...
const TOOL_DEFINITIONS: ToolDefinition[] = [
  {
    ...
    parameters: {
      ...
      properties: {
        text: {
          type: 'string',
          description: 'Text content to review. You may provide text alone or combine it with fileRef/fileRefs.',
        },
        fileRef: fileRefProperty('Single image, audio, or video FileRef to review.'),
        fileRefs: fileRefsProperty('Multiple image, audio, or video FileRefs to review together.'),
      },
      required: [],
    },
  },
...
async function fileRefToModerationInput(value: unknown, label: string): Promise<ModerationInputBlock> {
...
export const moderationFeature: SubFeature = {
  ...
  async execute(_toolName: string, input: Record<string, unknown>, config: Record<string, unknown>): Promise<string> {
    ...
    const moderationInput: string | ModerationInputBlock | ModerationInputBlock[] =
        ...
        for (const [index, item] of result.result_list.entries()) {
          lines.push(
            [
              ...
              item.risk_type?.length ? `risk_type=${item.risk_type.join(', ')}` : null,
            ].filter(Boolean).join('; '),
          );
        }
      }
