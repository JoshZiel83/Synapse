    ...
    parameters: {
      ...
      properties: {
        fileRef: fileRefProperty(
          "Image FileRef to send to the OCR service. The API expects an image such as JPG or PNG."
        ),
        languageType: {
          type: "string",
          description:
            "Optional OCR language/model hint. Use AUTO for automatic detection or CHN_ENG for mixed Chinese/English.",
          enum: LANGUAGE_TYPES,
        },
        probability: {
          type: "boolean",
          description:
            "Whether to request confidence information for each recognized text block.",
        } as any,
      },
      required: ["fileRef"],
    },
...
async function ensureSupportedOcrImage(
  buffer: Buffer,
  mimeType: string
...
export const ocrFeature: SubFeature = {
  ...
  async execute(
    ...
    const prepared = await ensureSupportedOcrImage(
      originalBuffer,
      record.mimeType
    )
    ...
    form.append(
      "file",
      new Blob([toBlobBytes(prepared.buffer)], { type: prepared.mimeType }),
      record.originalName || prepared.originalName
    )
