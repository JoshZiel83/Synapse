...
export const gitlabSeed: BuiltinOrgSeed = {
  slug: "gitlab",
  displayName: "GitLab",
  description: "GitLab official MCP integration with optional GitLab API-backed automation event sources.",
  plugins: [
    {
      slug: "official-mcp",
      displayName: "GitLab Official MCP",
      description: "Connect to GitLab's official MCP endpoint and reuse the same token for GitLab project event sources.",
      longDescription:
      ...
      validationRules: [
        { field: "apiKey", rule: "required", message: "GitLab token is required." },
        { field: "apiKey", rule: "min_length", value: 20, message: "GitLab token looks too short." },
      ],
      setupSteps: [
        {
          id: "credentials",
          title: "Configure GitLab",
          description: "Paste a GitLab token that can access the projects you need.",
          scope: "plugin",
          fields: ["apiKey", "baseUrl"],
          helpUrl: "https://docs.gitlab.com/user/gitlab_duo/model_context_protocol/",
