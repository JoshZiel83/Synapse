...
function buildAliases(value: string) {
  ...
  const aliases = new Set<string>([value, normalized, normalized.replace(/-/g, "_")]);
...
function createPropertySpec(input: {
  ...
  return {
    name: resolvedName,
    description:
      [readString(input.property.description), readString(input.property.desc_zh_cn)]
        .filter(Boolean)
        .join(" / ") || resolvedName,
    type,
...
function createActionSpec(input: {
  ...
  return {
    name: resolvedName,
    description:
      [readString(input.action.description), readString(input.action.desc_zh_cn)]
        .filter(Boolean)
        .join(" / ") || resolvedName,
    method: {
      siid: Number(input.serviceId),
...
async function fetchDeviceSpec(model: string): Promise<MijiaDeviceSpec> {
  ...
  const pageData = JSON.parse(decodeHtmlJsonPayload(match[1])) as Record<string, any>;
  ...
  for (const [serviceId, serviceValue] of Object.entries<Record<string, any>>(services)) {
    const serviceName = readString(serviceValue?.name, "service");
    for (const [propertyId, propertyValue] of Object.entries<Record<string, unknown>>(
    ...
    for (const [actionId, actionValue] of Object.entries<Record<string, unknown>>(
