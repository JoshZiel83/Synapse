...
import { decryptEncryptedPayload, genNonce, generateEncryptedParams, getSignedNonce } from "./crypto.js";
...
const SCENE_LIST_URI = "/appgateway/miot/appsceneservice/AppSceneService/GetSimpleSceneList";
const RUN_SCENE_URI = "/appgateway/miot/appsceneservice/AppSceneService/NewRunScene";
...
function formatMijiaErrorMessage(code: number, fallback?: string) {
  return fallback || MIJIA_ERROR_CODE_MESSAGES[String(code)] || `Mijia error ${code}`;
}
...
export class MijiaCloudClient {
  ...
  private readonly onAuthStateChanged?: (nextState: MijiaAuthState) => Promise<void> | void;
  ...
  private async encryptedRequest(uri: string, data: JsonObject) {
    ...
    if (!response.ok) {
      throw new MijiaApiError(
        ...
        { isAuthError: response.status === 401 || isMijiaAuthErrorCode(code), code },
      );
    }
    if (code !== 0 || payload.result === undefined) {
      throw new MijiaApiError(
        formatMijiaErrorMessage(code, readString(payload.message) || readString(payload.desc)),
  ...
  private async getHomeOwner(homeId: string) {
    ...
    const matched = homes.find((home: any) => String(home?.id) === String(homeId));
    if (!matched) {
  ...
  private buildHomeRoomLookup(homes: any[]) {
    const roomByDid = new Map<string, { homeId: string; homeName?: string; roomId?: string; roomName?: string }>();
  ...
  private mapDeviceRecord(
    device: any,
    roomLookup: Map<string, { homeId: string; homeName?: string; roomId?: string; roomName?: string }>,
    isShared: boolean,
  ): MijiaDeviceRecord {
