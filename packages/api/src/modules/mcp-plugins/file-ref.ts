...
export async function resolveFileRefRecord(value: unknown, label: string, expectedCategory?: FileCategory): Promise<FileRecord> {
  ...
  if (expectedCategory) {
    ...
    if (actualCategory !== expectedCategory) {
      throw new Error(`${label} must reference a ${expectedCategory} file, got ${actualCategory}`);
    }
  }
...
async function ensureSupportedVisionImage(buffer: Buffer, mimeType: string): Promise<{ buffer: Buffer; mimeType: string }> {
