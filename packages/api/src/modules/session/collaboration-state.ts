...
const PLAN_DRAFT_KEYS = new Set(["summary", "checklist", "explanation", "enteredAt"]);
...
function parsePlanDraftState(
  ...
  const summary =
    typeof value.summary === "string" ? value.summary.trim() || undefined : undefined;
  const explanation =
    typeof value.explanation === "string"
      ? value.explanation.trim() || undefined
      : undefined;
  const enteredAt =
    typeof value.enteredAt === "string" ? value.enteredAt.trim() || undefined : undefined;
...
export function parseSessionCollaborationState(
  ...
  return {
    planDraft: parsePlanDraftState(value.planDraft, "collaborationState.planDraft"),
  };
}
