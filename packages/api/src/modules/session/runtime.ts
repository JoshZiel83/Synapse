...
function getToolDisplayTitle(toolName: string, requestPayload?: Record<string, unknown>) {
  const visibleToolName = typeof requestPayload?.visibleToolName === 'string'
...
function mapToolActivityState(params: {
  ...
  if (params.toolCallStatus === 'failed' || params.latestResultIsError) return 'failed';
  return 'completed';
}
...
async function loadProcessingTargetsForTurn(turnId: string): Promise<ActorRuntimeProcessingTarget[]> {
  const rows = await db
    .selectFrom('session_wakeups')
    .select([
      'id',
...
function buildGenericToolResultBlocks(params: {
  ...
  if (finalErrorPayload && Object.keys(asRecord(finalErrorPayload)).length > 0) {
    return buildTextBlocksFromLines(prettyJson(finalErrorPayload));
  }
  if (finalResultPayload && Object.keys(asRecord(finalResultPayload)).length > 0) {
...
async function buildRelayBuiltinToolBlocks(params: {
  ...
  if (!builtinKind) {
    return {
      requestBlocks: buildGenericToolRequestBlocks(params.toolName, params.input),
  ...
  const visibleToolName = getToolDisplayTitle(params.toolName, params.requestPayload);
...
async function buildToolActivityDetail(turnId: string) {
  ...
  const outputChunksByTaskId = new Map<string, Array<{ stream: string; text_value: string }>>();
  ...
  if (toolCallIds.length > 0) {
    ...
    for (const row of tasks) {
      if (row.source_tool_call_id && !latestTasksByToolCall.has(row.source_tool_call_id)) {
  ...
  for (const toolCall of toolCalls) {
    ...
    const requestPayload = task ? parseMetadata(task.request_payload) : undefined;
    ...
    const blocks = toolCall.tool_kind === 'builtin'
...
function buildTurnPreviewFromDetail(
  ...
  const completedItems = detail.items.filter((item) => item.state === 'completed');
...
export async function buildSessionRuntimeSnapshot(
  ...
  const phase = overrides.phase
    || ((laneState === 'running' || laneState === 'queued' || laneState === 'blocked')
      ? cachedRuntime?.phase
      : undefined)
    || (laneState === 'running'
      ? 'thinking'
  ...
  const activeTurnId = overrides.activeTurnId
...
export async function enqueueSessionWakeup(params: {
  ...
  sourceParticipantType?: 'workspace_member' | 'actor' | 'remote_agent' | 'external' | 'system';
