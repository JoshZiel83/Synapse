...
type SherpaOnnxModule = {
  OfflineRecognizer: new (config: Record<string, unknown>) => {
    createStream(): { acceptWaveform(obj: { sampleRate: number; samples: Float32Array }): void };
    decode(stream: unknown): void;
    getResult(stream: unknown): Record<string, unknown>;
  };
  readWaveFromBinary(data: Uint8Array, enableExternalBuffer?: boolean): {
    sampleRate: number;
...
let recognizerPromise: Promise<InstanceType<SherpaOnnxModule['OfflineRecognizer']> | null> | null = null;
...
function guessExtension(block: AudioFileBlock): string {
  const byName = path.extname(block.originalName || '').replace('.', '').trim();
...
function getLocalSherpaConfig(): Record<string, unknown> | null {
  ...
  try {
    ...
    throw new Error(`invalid SHERPA_ONNX_CONFIG_JSON: ${err?.message || 'parse failed'}`);
  }
}
  ...
  if (!ffmpegModulePromise) {
    ffmpegModulePromise = Promise.resolve().then(() => {
      try {
        ...
        warnOnce('node-av/ffmpeg is not installed. Audio transcoding and local transcription fallback will be skipped.');
        return null;
      }
    });
  }
...
async function transcodeAudioToWave(block: AudioFileBlock, inputBuffer: Buffer): Promise<Buffer> {
  const ffmpeg = await loadNodeAvFfmpegModule();
  if (!ffmpeg) {
    throw new Error('skipped audio transcoding and transcription because node-av/ffmpeg is not installed');
  }

  if (!ffmpeg.isFfmpegAvailable()) {
    warnOnce('node-av ffmpeg binary is not available. Audio transcoding and local transcription fallback will be skipped.');
    throw new Error('skipped audio transcoding and transcription because the local node-av ffmpeg binary is not available');
  }
  ...
  try {
    ...
    throw new Error(stderr ? `audio transcode failed: ${stderr}` : `audio transcode failed: ${err?.message || 'unknown error'}`);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
}

async function getLocalRecognizer(): Promise<InstanceType<SherpaOnnxModule['OfflineRecognizer']> | null> {
...
async function transcribeWithLocalSherpaOnnx(block: AudioFileBlock): Promise<AudioTranscriptResult> {
  try {
    ...
    return {
      ok: false,
      error: err?.message || 'failed to transcribe audio with local sherpa-onnx',
    };
  }
}

async function transcribeWithSherpaOnnx(block: AudioFileBlock): Promise<AudioTranscriptResult> {
  return transcribeWithLocalSherpaOnnx(block);
}

async function getTranscript(block: AudioFileBlock): Promise<AudioTranscriptResult> {
...
export async function buildAudioFallbackContext(
  ...
  if (transcript.ok && transcript.text) {
    ...
    lines.push(`Reference transcript unavailable: ${transcript.error || 'unknown error'}.`);
  }

  lines.push(`If you need the original audio for another tool, pass the same FileRef ${fileRef} to that tool's fileRef parameter.`);
  return lines.join('\n');
}
