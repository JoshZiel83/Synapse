...
test(
  ...
  async () => {
    await withTestDbAndClient(async ({ db, client }) => {
      ...
      // Commit with a reconcile that FAILS (returns null) — simulates a syncDir
      // RPC error after the snapshot was appended.
      ...
      const result = await commitSpaces(sessionId, ["actor"], {
        ...
        reconcile: async (_m, _b, _c, hadConflicts) => {
          ...
          return null // force reconcile failure
        },
      })

      assert.equal(reconcileCalled, true, "reconcile was invoked")
      // A snapshot was appended (head advanced), reported as a conflict.
      assert.ok(
