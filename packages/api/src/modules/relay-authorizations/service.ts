  ...
  return {
    ...
    filesystem: grantSpec.filesystem
      ? {
          ...
          pathPrefixes: normalizePathPrefixes(grantSpec.filesystem.pathPrefixes),
        }
      : undefined,
    cua: grantSpec.cua
      ? {
    ...
    commandline: grantSpec.commandline
      ? {
          ...
          commandText: normalizeCommandText(grantSpec.commandline.commandText) || undefined,
          workingDirectory:
            normalizePathPrefix(grantSpec.commandline.workingDirectory) || undefined,
        }
      : undefined,
  };
}
...
function parseGrantSpec(value: unknown): RelayAuthorizationGrantSpec {
  const policy = parseJsonObject(value) as unknown as RelayAuthorizationGrantSpec;
...
function filesystemPolicyMatches(
  ...
  if (granted.pathPrefixes.length === 0 || requested.pathPrefixes.length === 0) {
    return false;
  }
...
function browserPolicyMatches(
  ...
  switch (granted.scopeType) {
    case "origin":
      return Boolean(granted.origin && requested.origin && granted.origin === requested.origin);
    case "host":
      return Boolean(granted.host && requested.host && granted.host === requested.host);
    case "domain":
      return Boolean(
...
function commandlinePolicyMatches(
  ...
  if (granted.workingDirectory) {
    const requestedWorkingDirectory = normalizePathPrefix(requested.workingDirectory);
    if (!requestedWorkingDirectory) {
      return false;
    }
    if (!pathWithinPrefix(requestedWorkingDirectory, granted.workingDirectory)) {
      return false;
    }
  }
