...
const fileUploadOriginSchema = z.object({
      ...
      for await (const part of request.parts()) {
        ...
        if (part.fieldname === 'origin' && typeof part.value === 'string') {
          try {
            ...
            return reply.status(400).send({ error: 'origin must be valid JSON' });
          }
        }
      }
    } catch {
...
export async function filesReadController(app: FastifyInstance) {
  ...
  app.post<{
    ...
    if (!runId) {
      return reply.status(400).send({ error: 'File type is not supported for default parsing' });
    }
  ...
  app.get<{
    ...
    const allowed = await canUserAccessFileWorkspace(info.workspaceId ?? null, userId);
    if (!allowed) {
      return reply.status(403).send({ error: 'Forbidden' });
    }
