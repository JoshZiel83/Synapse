...
export type DecodedProviderFrame = DecodedServerResponseFrame | DecodedErrorFrame;
...
export function decodeProviderFrame(frame: Buffer): DecodedProviderFrame {
  ...
  if (messageType === MESSAGE_TYPE_FULL_SERVER_RESPONSE) {
    ...
    if (flags === MESSAGE_FLAG_SEQUENCE || flags === MESSAGE_FLAG_FINAL_WITH_SEQUENCE) {
    ...
    return {
      ...
      sequence,
      isFinal:
        flags === MESSAGE_FLAG_FINAL || flags === MESSAGE_FLAG_FINAL_WITH_SEQUENCE,
      payloadSize,
      payload,
    };
  }
