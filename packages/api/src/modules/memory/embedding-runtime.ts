...
class MemoryEmbeddingRuntime {
  ...
  private buildWorker() {
    ...
    const workerUrl = new URL(`./embedding-worker.${sourceExt}`, import.meta.url);
  ...
  async warmup(options?: WarmupOptions) {
    ...
    this.warmupPromise = (async () => {
      try {
        ...
        this.state = 'warming';
        this.lastError = null;
        this.allowRemoteModels = options?.allowRemoteModels ?? config.memory.allowRuntimeModelDownload;
...
export async function warmMemoryEmbeddingRuntime(options?: WarmupOptions) {
  await runtime.warmup({
    allowRemoteModels: options?.allowRemoteModels ?? config.memory.allowRuntimeModelDownload,
  });
}
