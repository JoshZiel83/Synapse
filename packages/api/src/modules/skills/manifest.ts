...
function buildResolvedFrontmatter(params: {
  ...
  const normalizedName = normalizeSkillCommandName(rawName || params.fallbackName);
  if (!normalizedName) {
    throw new Error("Skill frontmatter must resolve to a non-empty name");
  }
  if (rawName && normalizedName !== rawName.trim()) {
    warnings.push(`Normalized frontmatter name "${rawName}" to "${normalizedName}"`);
  }
...
export function buildSkillContentHash(params: {
  frontmatter: SkillFrontmatter;
  bodyBlocks: CanonicalContentBlock[];
  files: Array<{ path: string; mediaType?: string; contentBlocks: CanonicalContentBlock[] }>;
}) {
  return sha256Hex(
    JSON.stringify({
      frontmatter: params.frontmatter,
      bodyBlocks: params.bodyBlocks,
      files: [...params.files].sort((left, right) => left.path.localeCompare(right.path)),
    }),
  );
}
...
export function buildSyntheticEntryFile(params: {
  ...
  return {
    ...
    contentBlocks: [textBlock(buildSkillMarkdown(params.frontmatter, params.bodyBlocks))],
  };
}
