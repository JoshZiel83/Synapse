...
let relayAutoSkillIndexPromise: Promise<RelayAutoSkillDefinition[]> | null =
  null
...
async function loadSkillDefinition(input: {
  ...
  const skillKey =
    capabilityKey(input.providerSlug, input.capabilitySlug) +
    `:${input.rawSkillSlug}`
...
async function loadVisibleHealthyRelayCommandlineExposureMetadata(input: {
  ...
  const { listVisibleHealthyRelayCommandlineExposureMetadata } =
    await import("../mcp-plugins/tool-resolver.js")
...
export async function listRelayAutoLoadedSkills(input: {
  ...
  try {
    ...
    console.error(
      "[relay-auto-skills] failed to load relay auto skills:",
      error
    )
    return [] as AvailableSkillSummary[]
  }
  ...
  for (const definition of definitions) {
    const key = capabilityKey(
      definition.providerSlug,
      definition.capabilitySlug
    )
...
export async function readRelayAutoLoadedSkill(input: {
  ...
  try {
    ...
    console.error(
      "[relay-auto-skills] failed to read relay auto skill index:",
      error
    )
    return null
  }
