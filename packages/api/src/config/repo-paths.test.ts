...
test("repo path helpers resolve from module location instead of cwd", async () => {
  ...
  try {
    ...
    assert.equal(
      existsSync(repoPaths.resolveRepoPath("relay", "managed-command-providers.json")),
      true,
    );
    assert.equal(
    ...
    assert.equal(
      existsSync(subprojects.resolveRepoSubprojectPath("notion-skills", "README.md")),
      true,
    );
    assert.equal(
      existsSync(subprojects.resolveRepoSubprojectPath("bilibili-cli", "SKILL.md")),
      true,
    );
    assert.equal(
