...
#
# Why one-file-per-invocation: every default test brings up its own
# isolated API process + reset's the shared synapse_test database in its
# `before` hook. If they ran concurrently (which is what
# `node --test <multiple-files>` does by default — files run in worker
# pools), they would race on the `DROP DATABASE / CREATE DATABASE` and on
# the single 127.0.0.1:38091 API port. We saw that race: pg_database_datname
# unique-violation + EADDRINUSE. Serializing the file list here means each
# file owns the DB and the API port for its full lifetime.
#
